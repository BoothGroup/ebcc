...
# Try to import TBLIS for NumPy backend
try:
    try:
        import tblis_einsum
    except ImportError:
...
# Define the size of problem to fall back on the built-in einsum of
# the chosen backend
...
def _fallback_einsum(*operands, **kwargs):
    """Handle the fallback to `np.einsum`."""
...
def contract(subscript, *args, **kwargs):
    ...
    **kwargs : dict
    ...
    """
    if FOUND_TBLIS and BACKEND_NAME == "numpy":
...
def _parse_einsum_input(operands):
    ...
    if isinstance(operands[0], str):
        ...
        for s in subscripts:
            if s in ".,->":
                continue
            if s not in numpy.core.einsumfunc.einsum_symbols:
        ...
        for num, sub in enumerate(subscript_list):
            for s in sub:
                if s is Ellipsis:
                    ...
                    subscripts += numpy.core.einsumfunc.einsum_symbols[s]
            if num != last:
                subscripts += ","
    ...
    if "." in subscripts:
        ...
        unused = list(numpy.core.einsumfunc.einsum_symbols_set - set(used))
