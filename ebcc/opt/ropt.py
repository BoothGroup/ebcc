"""Restricted orbital-optimised coupled cluster."""
...
if TYPE_CHECKING:
    ...
    from ebcc.cc.rebcc import REBCC, SpinArrayType, ERIsInputType
...
class _OptimisedREBCC(_BaseOptimisedEBCC):
    """Restricted orbital-optimised coupled cluster."""
...
class BruecknerREBCC(BaseBruecknerEBCC, _OptimisedREBCC):
...
class OptimisedREBCC(BaseOptimisedEBCC, _OptimisedREBCC):
    """Restricted optimised coupled cluster."""
    ...
    def get_rotation_matrix(
        ...
        x_vo = 0.5 * (f - np.transpose(f))[v, o] / self.cc.energy_sum("vo")
        x = _inflate(
        ...
        a: NDArray[T] = np.asarray(np.real(scipy.linalg.logm(u_tot)), dtype=types[float])
        if damping is not None:
            a = damping(a)  # FIXME can x be the error?
