...
with common.FilePrinter("%sCC%s" % (prefix.upper(), "_".join(rank).rstrip("_"))) as file_printer:
    ...
    if spin != "uhf":  # TODO
        #for tag in ("", "_o", "_v"):
        #    particles["bra1"+tag] = ((codegen.FERMION, 0), (codegen.FERMION, 0))
        #    particles["ket1"+tag] = ((codegen.FERMION, 0), (codegen.FERMION, 0))
        #    particles["bra2"+tag] = ((codegen.FERMION, 0), (codegen.FERMION, 1), (codegen.FERMION, 0), (codegen.FERMION, 1))
        #    particles["ket2"+tag] = ((codegen.FERMION, 0), (codegen.FERMION, 1), (codegen.FERMION, 0), (codegen.FERMION, 1))

        #particles["r1"] = ((codegen.FERMION, 0),)
        #particles["r2"] = ((codegen.FERMION, 0), (codegen.FERMION, 1), (codegen.FERMION, 0))
        #particles["r1new"] = ((codegen.FERMION, 0),)
        #particles["r2new"] = ((codegen.FERMION, 0), (codegen.FERMION, 1), (codegen.FERMION, 0))
        ...
        for ket, ket_name in [(True, "ket"), (False, "bra")]:
            for ip, ip_name in [(True, "ip"), (False, "ea")]:
                with FunctionPrinter(
                    ...
                    if ket and not ip:
                        function_printer.write_python("    ket2 = ket2.swapaxes(0, 1)\n")
        ...
        for ip, ip_name in [(True, "ip"), (False, "ea")]:
            with FunctionPrinter(
                ...
                for space_no, (bra, ket) in enumerate(zip(bras, kets)):
                    ...
                    terms, indices = codegen.wick_to_sympy(expr, particles, return_value=return_value)
                    terms = transform_spin(terms, indices)
                    terms = [codegen.sympy_to_drudge(group, indices, dr=dr, restricted=spin!="uhf", skip_symmetry={"r1", "r2"}) for group in terms]
                    terms = codegen.spin_integrate._flatten([terms])
                ...
                if not ip:
                    function_printer.write_python("    r2 = r2.swapaxes(0, 1)\n")
        ...
        for ip, ip_name in [(True, "ip"), (False, "ea")]:
            with FunctionPrinter(
                ...
                excitation = sum(excitations, Expression([]))
                ...
                for space_no, space in enumerate(spaces):
                    ...
                    terms = [codegen.sympy_to_drudge(
                            ...
                            dr=dr,
                            restricted=spin!="uhf",
                            skip_symmetry={return_value, return_value+"new"},
                    ) for group in terms]
                    all_terms.append(terms)
                ...
                if not ip:
                    function_printer.write_python("    r2new = r2new.swapaxes(0, 1)\n")
