...
def energy_perturbative(f=None, v=None, nocc=None, nvir=None, t1=None, t2=None, l1=None, l2=None, **kwargs):
    ...
    x0 += np.einsum("ljab,ilkc->jabikc", t2, v.ooov)
    t3 = np.zeros((nocc, nocc, nocc, nvir, nvir, nvir), dtype=np.float64)
    t3 += np.einsum("jabikc->ijkabc", x0) * -1.0
    t3 += np.einsum("jabkic->ijkabc", x0)
    t3 += np.einsum("jcbika->ijkabc", x0)
    t3 += np.einsum("jcbkia->ijkabc", x0) * -1.0
    del x0
    x1 = np.zeros((nocc, nocc, nvir, nocc, nvir, nvir), dtype=np.float64)
    x1 += np.einsum("ikad,jbcd->ikajbc", t2, v.ovvv)
    t3 += np.einsum("ijakcb->ijkabc", x1)
    t3 += np.einsum("ijckab->ijkabc", x1) * -1.0
    t3 += np.einsum("ikajbc->ijkabc", x1)
    t3 += np.einsum("ikcjba->ijkabc", x1) * -1.0
    t3 += np.einsum("kjaicb->ijkabc", x1) * -1.0
    t3 += np.einsum("kjciab->ijkabc", x1)
    del x1
    x2 = np.zeros((nocc, nvir, nvir, nocc, nocc, nvir), dtype=np.float64)
    x2 += np.einsum("ilab,jlkc->iabjkc", t2, v.ooov)
    t3 += np.einsum("iabjkc->ijkabc", x2) * -1.0
    t3 += np.einsum("iackjb->ijkabc", x2) * -1.0
    t3 += np.einsum("icakjb->ijkabc", x2)
    t3 += np.einsum("icbjka->ijkabc", x2)
    t3 += np.einsum("kabjic->ijkabc", x2)
    t3 += np.einsum("kacijb->ijkabc", x2)
    t3 += np.einsum("kcaijb->ijkabc", x2) * -1.0
    t3 += np.einsum("kcbjia->ijkabc", x2) * -1.0
    del x2
    x3 = np.zeros((nocc, nocc, nvir, nocc, nvir, nvir), dtype=np.float64)
    x3 += np.einsum("ikdc,jbad->ikcjba", t2, v.ovvv)
    t3 += np.einsum("ijbkac->ijkabc", x3) * -1.0
    t3 += np.einsum("ijbkca->ijkabc", x3)
    t3 += np.einsum("ikajbc->ijkabc", x3) * -1.0
    t3 += np.einsum("ikcjba->ijkabc", x3)
    t3 += np.einsum("kjbiac->ijkabc", x3)
    t3 += np.einsum("kjbica->ijkabc", x3) * -1.0
    del x3

    # energy
    x0 = np.zeros((nvir, nocc, nocc, nvir), dtype=np.float64)
    x0 += np.einsum("kcad,ijkdbc->aijb", v.ovvv, t3)
    x1 = np.zeros((nocc, nocc, nocc, nvir), dtype=np.float64)
    x1 += np.einsum("abij,jklbac->iklc", l2, t3)
    x2 = np.zeros((nocc, nvir), dtype=np.float64)
    x2 += np.einsum("jbkc,ijkbac->ia", v.ovov, t3)
    x3 = np.zeros((nocc, nvir, nocc, nvir), dtype=np.float64)
    x3 += np.einsum("jbkc->jbkc", v.ovov)
    x3 += np.einsum("jckb->jbkc", v.ovov) * -1.0
    x4 = np.zeros((nvir, nvir, nocc, nocc), dtype=np.float64)
    x4 += np.einsum("abij->abij", l2)
    x4 += np.einsum("baij->abij", l2) * -1.0
    x5 = np.zeros((nocc, nvir, nocc, nvir), dtype=np.float64)
    x5 += np.einsum("jbkc->jckb", v.ovov) * -2.0
    x5 += np.einsum("jckb->jckb", x3)
    del x3
    x6 = np.zeros((nvir, nocc, nocc, nvir), dtype=np.float64)
    x6 += np.einsum("kcbd,ikjacd->bija", v.ovvv, t3)
    x7 = np.zeros((nocc, nocc, nocc, nvir), dtype=np.float64)
    x7 += np.einsum("abij,jklacb->iklc", l2, t3)
    x8 = np.zeros((nocc, nocc, nocc, nvir), dtype=np.float64)
    x8 += np.einsum("iklc->ilkc", x1) * -1.0
    x8 += np.einsum("ilkc->ilkc", x7)
    x9 = np.zeros((nocc, nocc, nocc, nvir), dtype=np.float64)
    x9 += np.einsum("abij,iklacb->jklc", l2, t3)
    x10 = np.zeros((nocc, nocc, nocc, nvir), dtype=np.float64)
    x10 += np.einsum("jklc->jklc", v.ooov)
    x10 += np.einsum("jlkc->jklc", v.ooov) * -1.0
    x11 = np.zeros((nocc, nocc, nocc, nvir), dtype=np.float64)
    x11 += np.einsum("abij,jklabc->iklc", l2, t3)
    x12 = np.zeros((nocc, nocc, nocc, nvir), dtype=np.float64)
    x12 += np.einsum("iklc->iklc", x11)
    x12 += np.einsum("iklc->iklc", x7) * -1.0
    del x7
    x13 = np.zeros((nvir, nocc, nocc, nvir), dtype=np.float64)
    x13 += np.einsum("kcbd,ijkacd->bija", v.ovvv, t3)
    x14 = np.zeros((nvir, nocc, nocc, nvir), dtype=np.float64)
    x14 += np.einsum("bija->bija", x13)
    x14 += np.einsum("bija->bija", x6) * -1.0
    x15 = np.zeros((nvir, nocc, nocc, nvir), dtype=np.float64)
    x15 += np.einsum("kcad,jikbdc->ajib", v.ovvv, t3)
    x16 = np.zeros((nocc, nocc, nocc, nvir), dtype=np.float64)
    x16 += np.einsum("abij,iklabc->jklc", l2, t3)
    x17 = np.zeros((nocc, nocc, nocc, nvir), dtype=np.float64)
    x17 += np.einsum("jklc->jklc", x16)
    x17 += np.einsum("jlkc->jklc", x9)
    x18 = np.zeros((nvir, nocc, nocc, nvir), dtype=np.float64)
    x18 += np.einsum("kcad,ijkcbd->aijb", v.ovvv, t3)
    x19 = np.zeros((nvir, nocc, nocc, nvir), dtype=np.float64)
    x19 += np.einsum("aijb->aijb", x0) * -1.0
    del x0
    x19 += np.einsum("aijb->aijb", x18)
    del x18
    x20 = np.zeros((nvir, nocc, nocc, nvir), dtype=np.float64)
    x20 += np.einsum("bjia->bjia", x19)
    x20 += np.einsum("bjia->bjia", x6)
    del x6
    x21 = np.zeros((nocc, nocc, nocc, nvir, nvir, nvir), dtype=np.float64)
    x21 += np.einsum("iklbac->iklbac", t3)
    x21 += np.einsum("kilbac->iklbac", t3) * -1.0
    x22 = np.zeros((nocc, nocc, nocc, nvir, nvir, nvir), dtype=np.float64)
    x22 += np.einsum("jikbac->ijkcab", t3)
    x22 += np.einsum("ijkcab->ijkcab", x21)
    x23 = np.zeros((nocc, nocc, nocc, nvir), dtype=np.float64)
    x23 += np.einsum("abij,iklbac->jklc", l2, x21)
    del x21
    x24 = 0
    x24 += np.einsum("jklc,jklc->", x10, x23)
    del x10, x23
    e_pert = 0
    e_pert += np.einsum("->", x24) * 0.5
    del x24
    x25 = np.zeros((nvir, nocc, nocc, nvir), dtype=np.float64)
    x25 += np.einsum("bjia->bjia", x13)
    del x13
    x25 += np.einsum("bija->bjia", x14) * -1.0
    del x14
    x26 = np.zeros((nvir, nocc, nocc, nvir), dtype=np.float64)
    x26 += np.einsum("bija->bjia", x15) * 2.0
    x26 += np.einsum("bjia->bjia", x25)
    del x25
    x27 = np.zeros((nvir, nocc, nocc, nvir), dtype=np.float64)
    x27 += np.einsum("bjia->bjia", x20) * -1.0
    del x20
    x27 += np.einsum("bjia->bjia", x26)
    del x26
    x28 = 0
    x28 += np.einsum("abij,bjia->", l2, x27)
    del x27
    e_pert += np.einsum("->", x28)
    del x28
    x29 = np.zeros((nocc, nocc, nocc, nvir), dtype=np.float64)
    x29 += np.einsum("ilkc->ilkc", x1) * -1.0
    del x1
    x29 += np.einsum("ilkc->ilkc", x11)
    del x11
    x30 = np.zeros((nocc, nvir), dtype=np.float64)
    x30 += np.einsum("jckb,ijkabc->ia", x5, t3)
    del x5
    x31 = np.zeros((nocc, nvir), dtype=np.float64)
    x31 += np.einsum("jbkc,ijkcab->ia", v.ovov, x22)
    del x22
    x32 = np.zeros((nocc, nvir), dtype=np.float64)
    x32 += np.einsum("ia->ia", x30) * 2.0
    del x30
    x32 += np.einsum("ia->ia", x31) * -1.0
    del x31
    x33 = np.zeros((nocc, nvir), dtype=np.float64)
    x33 += np.einsum("ia->ia", x2)
    del x2
    x33 += np.einsum("ia->ia", x32)
    del x32
    x34 = 0
    x34 += np.einsum("ai,ia->", l1, x33)
    del x33
    e_pert += np.einsum("->", x34) * -0.5
    del x34
    x35 = np.zeros((nvir, nocc, nocc, nvir), dtype=np.float64)
    x35 += np.einsum("ajib->ajib", x15) * 2.0
    del x15
    x35 += np.einsum("aijb->ajib", x19) * -1.0
    del x19
    x36 = 0
    x36 += np.einsum("ajib,abij->", x35, x4)
    del x35, x4
    e_pert += np.einsum("->", x36) * 0.5
    del x36
    x37 = np.zeros((nocc, nocc, nocc, nvir), dtype=np.float64)
    x37 += np.einsum("abij,kjlabc->iklc", l2, t3)
    x38 = np.zeros((nocc, nocc, nocc, nvir), dtype=np.float64)
    x38 += np.einsum("iklc->iklc", x12)
    del x12
    x38 += np.einsum("iklc->iklc", x37) * -1.0
    x39 = np.zeros((nocc, nocc, nocc, nvir), dtype=np.float64)
    x39 += np.einsum("ilkc->ilkc", x37)
    del x37
    x39 += np.einsum("iklc->ilkc", x9)
    del x9
    x40 = np.zeros((nocc, nocc, nocc, nvir), dtype=np.float64)
    x40 += np.einsum("ilkc->ilkc", x29)
    del x29
    x40 += np.einsum("iklc->ilkc", x38) * -1.0
    del x38
    x41 = np.zeros((nocc, nocc, nocc, nvir), dtype=np.float64)
    x41 += np.einsum("ilkc->ilkc", x40) * -1.0
    del x40
    x41 += np.einsum("ilkc->ilkc", x8) * 3.0
    del x8
    x42 = np.zeros((nocc, nocc, nocc, nvir), dtype=np.float64)
    x42 += np.einsum("jlkc->jlkc", x16)
    del x16
    x42 += np.einsum("jlkc->jlkc", x39)
    del x39
    x43 = np.zeros((nocc, nocc, nocc, nvir), dtype=np.float64)
    x43 += np.einsum("jklc->jklc", x17) * 3.0
    del x17
    x43 += np.einsum("jlkc->jklc", x42) * -1.0
    del x42
    x44 = np.zeros((nocc, nocc, nocc, nvir), dtype=np.float64)
    x44 += np.einsum("ilkc->ilkc", x41)
    del x41
    x44 += np.einsum("iklc->ilkc", x43) * -1.0
    del x43
    x45 = 0
    x45 += np.einsum("iklc,ilkc->", v.ooov, x44)
    del x44
    e_pert += np.einsum("->", x45) * 0.5
    del x45
