"""
Code generated by `albert`:
https://github.com/obackhouse/albert

  * date: 2024-10-18T21:45:51.654651
  * python version: 3.10.15 (main, Sep  9 2024, 03:02:45) [GCC 11.4.0]
  * albert version: 0.0.0
  * caller: /opt/hostedtoolcache/Python/3.10.15/x64/lib/python3.10/site-packages/albert/codegen/einsum.py
  * node: fv-az1016-35
  * system: Linux
  * processor: x86_64
  * release: 6.5.0-1025-azure
"""

from ebcc import numpy as np
from ebcc.util import pack_2e, einsum, dirsum, Namespace


def energy(t2=None, v=None, **kwargs):
    """
    Code generated by `albert` 0.0.0 on 2024-10-18T21:46:02.333275.

    Parameters
    ----------
    t2 : array
        T2 amplitudes.
    v : array
        Electron repulsion integrals.

    Returns
    -------
    e_cc : float
        Coupled cluster energy.
    """

    tmp0 = np.copy(np.transpose(t2, (0, 1, 3, 2))) * -1
    tmp0 += t2 * 2
    tmp1 = einsum(v.xov, (0, 1, 2), tmp0, (1, 3, 2, 4), (3, 4, 0)) * 0.5
    del tmp0
    e_cc = einsum(tmp1, (0, 1, 2), v.xov, (2, 0, 1), ()) * 2
    del tmp1

    return e_cc

def update_amps(f=None, t2=None, v=None, **kwargs):
    """
    Code generated by `albert` 0.0.0 on 2024-10-18T21:47:45.554725.

    Parameters
    ----------
    f : array
        Fock matrix.
    t2 : array
        T2 amplitudes.
    v : array
        Electron repulsion integrals.

    Returns
    -------
    t2new : array
        Updated T2 residuals.
    """

    tmp0 = einsum(v.xov, (0, 1, 2), v.xov, (0, 3, 4), (3, 1, 4, 2))
    t2new = np.copy(np.transpose(tmp0, (1, 0, 3, 2)))
    tmp1 = einsum(v.xvv, (0, 1, 2), v.xvv, (0, 3, 4), (1, 3, 4, 2))
    t2new += einsum(t2, (0, 1, 2, 3), tmp1, (4, 3, 5, 2), (0, 1, 4, 5))
    del tmp1
    tmp2 = einsum(v.xov, (0, 1, 2), t2, (3, 1, 2, 4), (3, 4, 0))
    t2new += einsum(tmp2, (0, 1, 2), tmp2, (3, 4, 2), (0, 3, 1, 4))
    tmp3 = einsum(t2, (0, 1, 2, 3), tmp0, (1, 4, 5, 2), (0, 4, 3, 5))
    t2new += einsum(t2, (0, 1, 2, 3), tmp3, (4, 1, 5, 2), (4, 0, 3, 5))
    tmp4 = einsum(f.vv, (0, 1), t2, (2, 3, 4, 1), (2, 3, 0, 4))
    tmp7 = np.copy(tmp4)
    del tmp4
    tmp5 = einsum(v.xvv, (0, 1, 2), v.xoo, (0, 3, 4), (3, 4, 1, 2))
    tmp6 = einsum(t2, (0, 1, 2, 3), tmp5, (4, 1, 5, 2), (0, 4, 3, 5))
    tmp7 += tmp6 * -1
    del tmp6
    t2new += np.transpose(tmp7, (0, 1, 3, 2))
    t2new += np.transpose(tmp7, (1, 0, 2, 3))
    del tmp7
    tmp8 = einsum(v.xov, (0, 1, 2), tmp2, (3, 4, 0), (3, 1, 4, 2))
    tmp13 = np.copy(tmp8)
    del tmp8
    tmp11 = np.copy(np.transpose(tmp5, (1, 0, 3, 2)))
    del tmp5
    tmp11 += tmp3 * -1
    del tmp3
    tmp9 = np.copy(np.transpose(v.xov, (1, 2, 0)))
    tmp9 += tmp2 * -1
    del tmp2
    tmp10 = einsum(tmp9, (0, 1, 2), v.xov, (2, 3, 4), (3, 0, 4, 1))
    del tmp9
    tmp11 += np.transpose(tmp10, (1, 0, 3, 2)) * -2
    del tmp10
    tmp12 = einsum(tmp11, (0, 1, 2, 3), t2, (4, 1, 5, 3), (4, 0, 5, 2))
    del tmp11
    tmp13 += tmp12
    del tmp12
    t2new += tmp13 * -1
    t2new += np.transpose(tmp13, (1, 0, 3, 2)) * -1
    del tmp13
    tmp14 = einsum(f.oo, (0, 1), t2, (2, 1, 3, 4), (0, 2, 3, 4))
    t2new += np.transpose(tmp14, (0, 1, 3, 2)) * -1
    t2new += np.transpose(tmp14, (1, 0, 2, 3)) * -1
    del tmp14
    tmp15 = np.copy(np.transpose(t2, (0, 1, 3, 2))) * -1
    tmp15 += t2 * 2
    tmp16 = einsum(v.xov, (0, 1, 2), tmp15, (1, 3, 2, 4), (3, 4, 0))
    del tmp15
    tmp17 = einsum(tmp16, (0, 1, 2), v.xov, (2, 0, 3), (3, 1)) * 0.5
    tmp18 = einsum(tmp17, (0, 1), t2, (2, 3, 0, 4), (2, 3, 4, 1)) * 2
    del tmp17
    tmp21 = np.copy(np.transpose(tmp18, (1, 0, 2, 3)))
    del tmp18
    tmp19 = einsum(tmp16, (0, 1, 2), v.xov, (2, 3, 1), (3, 0))
    del tmp16
    tmp20 = einsum(tmp19, (0, 1), t2, (2, 0, 3, 4), (2, 1, 4, 3))
    del tmp19
    tmp21 += np.transpose(tmp20, (0, 1, 3, 2))
    del tmp20
    t2new += tmp21 * -1
    t2new += np.transpose(tmp21, (1, 0, 3, 2)) * -1
    del tmp21
    tmp22 = einsum(v.xoo, (0, 1, 2), v.xoo, (0, 3, 4), (3, 1, 2, 4))
    tmp22 += einsum(t2, (0, 1, 2, 3), tmp0, (4, 5, 3, 2), (4, 0, 5, 1))
    t2new += einsum(tmp22, (0, 1, 2, 3), t2, (0, 2, 4, 5), (1, 3, 5, 4))
    del tmp22
    tmp23 = np.copy(np.transpose(tmp0, (1, 0, 2, 3))) * 2
    tmp23 += np.transpose(tmp0, (1, 0, 3, 2)) * -1
    del tmp0
    tmp24 = einsum(t2, (0, 1, 2, 3), tmp23, (1, 4, 5, 3), (0, 4, 2, 5))
    del tmp23
    t2new += einsum(tmp24, (0, 1, 2, 3), t2, (4, 1, 5, 3), (0, 4, 2, 5)) * 2
    del tmp24

    return {f"t2new": t2new}

def update_lams(f=None, l2=None, t2=None, v=None, **kwargs):
    """
    Code generated by `albert` 0.0.0 on 2024-10-18T21:49:46.093135.

    Parameters
    ----------
    f : array
        Fock matrix.
    l2 : array
        L2 amplitudes.
    t2 : array
        T2 amplitudes.
    v : array
        Electron repulsion integrals.

    Returns
    -------
    l2new : array
        Updated L2 residuals.
    """

    tmp0 = einsum(v.xov, (0, 1, 2), v.xov, (0, 3, 4), (3, 1, 4, 2))
    l2new = np.copy(np.transpose(tmp0, (3, 2, 1, 0)))
    tmp1 = einsum(v.xoo, (0, 1, 2), v.xoo, (0, 3, 4), (3, 1, 2, 4))
    l2new += einsum(tmp1, (0, 1, 2, 3), l2, (4, 5, 3, 1), (4, 5, 0, 2))
    del tmp1
    tmp2 = einsum(v.xvv, (0, 1, 2), v.xvv, (0, 3, 4), (1, 3, 4, 2))
    l2new += einsum(l2, (0, 1, 2, 3), tmp2, (4, 0, 5, 1), (5, 4, 2, 3))
    del tmp2
    tmp3 = einsum(t2, (0, 1, 2, 3), l2, (3, 2, 4, 5), (5, 4, 0, 1))
    l2new += einsum(tmp0, (0, 1, 2, 3), tmp3, (4, 5, 0, 1), (3, 2, 5, 4))
    del tmp3
    tmp4 = einsum(tmp0, (0, 1, 2, 3), t2, (4, 5, 2, 3), (4, 5, 0, 1))
    l2new += einsum(l2, (0, 1, 2, 3), tmp4, (2, 3, 4, 5), (0, 1, 4, 5))
    del tmp4
    tmp5 = einsum(f.vv, (0, 1), l2, (2, 1, 3, 4), (3, 4, 0, 2))
    tmp10 = np.copy(tmp5)
    del tmp5
    tmp6 = einsum(v.xvv, (0, 1, 2), v.xoo, (0, 3, 4), (3, 4, 1, 2))
    tmp8 = np.copy(np.transpose(tmp6, (1, 0, 3, 2))) * -1
    tmp7 = einsum(tmp0, (0, 1, 2, 3), t2, (4, 1, 2, 5), (4, 0, 5, 3))
    tmp8 += tmp7
    del tmp7
    tmp9 = einsum(tmp8, (0, 1, 2, 3), l2, (4, 2, 0, 5), (5, 1, 4, 3))
    del tmp8
    tmp10 += tmp9
    del tmp9
    l2new += np.transpose(tmp10, (3, 2, 0, 1))
    l2new += np.transpose(tmp10, (2, 3, 1, 0))
    del tmp10
    tmp11 = einsum(tmp6, (0, 1, 2, 3), l2, (4, 3, 5, 1), (5, 0, 4, 2))
    del tmp6
    tmp22 = np.copy(tmp11)
    del tmp11
    tmp12 = einsum(t2, (0, 1, 2, 3), l2, (4, 2, 5, 1), (5, 0, 4, 3))
    tmp15 = np.copy(tmp12)
    del tmp12
    tmp13 = np.copy(np.transpose(l2, (3, 2, 0, 1))) * -1
    tmp13 += np.transpose(l2, (2, 3, 0, 1)) * 2
    tmp14 = einsum(tmp13, (0, 1, 2, 3), t2, (4, 0, 5, 2), (4, 1, 5, 3))
    tmp15 += np.transpose(tmp14, (1, 0, 3, 2)) * -1
    del tmp14
    tmp16 = einsum(tmp0, (0, 1, 2, 3), tmp15, (4, 0, 5, 3), (1, 4, 2, 5))
    del tmp0, tmp15
    tmp22 += np.transpose(tmp16, (1, 0, 3, 2)) * -1
    del tmp16
    tmp19 = np.copy(np.transpose(v.xov, (1, 2, 0)))
    tmp17 = np.copy(np.transpose(t2, (0, 1, 3, 2))) * -1
    tmp17 += t2 * 2
    tmp18 = einsum(v.xov, (0, 1, 2), tmp17, (1, 3, 2, 4), (3, 4, 0))
    del tmp17
    tmp19 += tmp18
    del tmp18
    tmp20 = einsum(tmp19, (0, 1, 2), tmp13, (0, 3, 1, 4), (3, 4, 2))
    del tmp13, tmp19
    tmp21 = einsum(tmp20, (0, 1, 2), v.xov, (2, 3, 4), (3, 0, 4, 1))
    del tmp20
    tmp22 += np.transpose(tmp21, (1, 0, 3, 2)) * -1
    del tmp21
    l2new += np.transpose(tmp22, (2, 3, 0, 1)) * -1
    l2new += np.transpose(tmp22, (3, 2, 1, 0)) * -1
    del tmp22
    tmp23 = einsum(f.oo, (0, 1), l2, (2, 3, 4, 1), (0, 4, 2, 3))
    l2new += np.transpose(tmp23, (3, 2, 0, 1)) * -1
    l2new += np.transpose(tmp23, (2, 3, 1, 0)) * -1
    del tmp23
    tmp24 = np.copy(np.transpose(t2, (0, 1, 3, 2)))
    tmp24 += t2 * -0.5
    tmp25 = einsum(tmp24, (0, 1, 2, 3), l2, (3, 2, 0, 4), (4, 1)) * 2
    tmp26 = einsum(v.xov, (0, 1, 2), tmp25, (3, 1), (3, 2, 0)) * 0.5
    del tmp25
    tmp27 = einsum(tmp26, (0, 1, 2), v.xov, (2, 3, 4), (3, 0, 4, 1)) * 2
    del tmp26
    tmp31 = np.copy(np.transpose(tmp27, (1, 0, 2, 3)))
    del tmp27
    tmp28 = einsum(tmp24, (0, 1, 2, 3), l2, (4, 2, 0, 1), (4, 3)) * 2
    del tmp24
    tmp29 = einsum(v.xov, (0, 1, 2), tmp28, (3, 2), (1, 3, 0))
    del tmp28
    tmp30 = einsum(tmp29, (0, 1, 2), v.xov, (2, 3, 4), (3, 0, 4, 1))
    del tmp29
    tmp31 += np.transpose(tmp30, (0, 1, 3, 2))
    del tmp30
    l2new += np.transpose(tmp31, (3, 2, 0, 1)) * -1
    l2new += np.transpose(tmp31, (2, 3, 1, 0)) * -1
    del tmp31
    tmp32 = np.copy(np.transpose(t2, (0, 1, 3, 2))) * -1
    tmp32 += t2 * 2
    tmp33 = einsum(v.xov, (0, 1, 2), tmp32, (1, 3, 2, 4), (3, 4, 0))
    del tmp32
    tmp34 = einsum(v.xov, (0, 1, 2), tmp33, (1, 3, 0), (2, 3)) * 0.5
    tmp35 = einsum(l2, (0, 1, 2, 3), tmp34, (4, 1), (3, 2, 0, 4)) * 2
    del tmp34
    tmp38 = np.copy(np.transpose(tmp35, (1, 0, 2, 3)))
    del tmp35
    tmp36 = einsum(tmp33, (0, 1, 2), v.xov, (2, 3, 1), (3, 0))
    del tmp33
    tmp37 = einsum(tmp36, (0, 1), l2, (2, 3, 1, 4), (4, 0, 2, 3))
    del tmp36
    tmp38 += np.transpose(tmp37, (0, 1, 3, 2))
    del tmp37
    l2new += np.transpose(tmp38, (2, 3, 0, 1)) * -1
    l2new += np.transpose(tmp38, (3, 2, 1, 0)) * -1
    del tmp38

    return {f"l2new": l2new}

def make_rdm1_f(l2=None, t2=None, **kwargs):
    """
    Code generated by `albert` 0.0.0 on 2024-10-18T21:49:54.815125.

    Parameters
    ----------
    l2 : array
        L2 amplitudes.
    t2 : array
        T2 amplitudes.

    Returns
    -------
    rdm1 : array
        One-particle reduced density matrix.
    """

    rdm1 = Namespace()
    delta = Namespace(
        oo=np.eye(t2.shape[0]),
        vv=np.eye(t2.shape[-1]),
    )
    rdm1.oo = np.copy(delta.oo) * 2
    del delta
    tmp0 = np.copy(np.transpose(t2, (0, 1, 3, 2)))
    tmp0 += t2 * -0.5
    rdm1.oo += einsum(tmp0, (0, 1, 2, 3), l2, (3, 2, 0, 4), (1, 4)) * -4
    del tmp0
    tmp1 = np.copy(np.transpose(t2, (0, 1, 3, 2))) * -0.5
    tmp1 += t2
    rdm1.vv = einsum(l2, (0, 1, 2, 3), tmp1, (3, 2, 1, 4), (0, 4)) * 4
    del tmp1
    rdm1.ov = np.zeros((t2.shape[0], t2.shape[-1]))
    rdm1.vo = np.zeros((t2.shape[-1], t2.shape[0]))
    rdm1 = np.block([[rdm1.oo, rdm1.ov], [rdm1.vo, rdm1.vv]])

    return rdm1

def make_rdm2_f(l2=None, t2=None, **kwargs):
    """
    Code generated by `albert` 0.0.0 on 2024-10-18T21:53:52.268327.

    Parameters
    ----------
    l2 : array
        L2 amplitudes.
    t2 : array
        T2 amplitudes.

    Returns
    -------
    rdm2 : array
        Two-particle reduced density matrix.
    """

    rdm2 = Namespace()
    delta = Namespace(
        oo=np.eye(t2.shape[0]),
        vv=np.eye(t2.shape[-1]),
    )
    rdm2.vvoo = np.copy(np.transpose(l2, (0, 1, 3, 2))) * -2
    rdm2.vvoo += l2 * 4
    tmp28 = einsum(l2, (0, 1, 2, 3), t2, (3, 2, 4, 5), (0, 1, 5, 4))
    rdm2.vvvv = np.copy(np.transpose(tmp28, (1, 0, 2, 3))) * -2
    rdm2.vvvv += np.transpose(tmp28, (1, 0, 3, 2)) * 4
    del tmp28
    rdm2.oooo = einsum(delta.oo, (0, 1), delta.oo, (2, 3), (0, 2, 1, 3)) * 4
    rdm2.oooo += einsum(delta.oo, (0, 1), delta.oo, (2, 3), (2, 0, 1, 3)) * -2
    tmp0 = einsum(t2, (0, 1, 2, 3), l2, (3, 2, 4, 5), (5, 4, 0, 1))
    rdm2.oooo += np.transpose(tmp0, (2, 3, 1, 0)) * -2
    rdm2.oooo += np.transpose(tmp0, (3, 2, 1, 0)) * 4
    tmp1 = np.copy(np.transpose(t2, (0, 1, 3, 2)))
    tmp1 += t2 * -0.5
    tmp2 = einsum(tmp1, (0, 1, 2, 3), l2, (3, 2, 0, 4), (4, 1)) * 2
    del tmp1
    rdm2.oooo += einsum(delta.oo, (0, 1), tmp2, (2, 3), (3, 0, 2, 1)) * -4
    rdm2.oooo += einsum(delta.oo, (0, 1), tmp2, (2, 3), (0, 3, 2, 1)) * 2
    rdm2.oooo += einsum(tmp2, (0, 1), delta.oo, (2, 3), (1, 2, 3, 0)) * 2
    rdm2.oooo += einsum(delta.oo, (0, 1), tmp2, (2, 3), (0, 3, 1, 2)) * -4
    tmp21 = np.copy(np.transpose(t2, (0, 1, 3, 2)))
    tmp21 += t2 * -1
    tmp22 = einsum(l2, (0, 1, 2, 3), tmp21, (2, 4, 5, 1), (3, 4, 0, 5))
    del tmp21
    rdm2.ovov = np.copy(np.transpose(tmp22, (1, 2, 0, 3))) * 2
    tmp23 = np.copy(np.transpose(t2, (0, 1, 3, 2))) * 2
    tmp23 += t2 * -1
    tmp24 = einsum(l2, (0, 1, 2, 3), tmp23, (3, 4, 5, 1), (2, 4, 0, 5))
    del tmp23
    rdm2.ovov += np.transpose(tmp24, (1, 2, 0, 3)) * -2
    tmp12 = np.copy(np.transpose(l2, (3, 2, 0, 1))) * 2
    tmp12 += np.transpose(l2, (2, 3, 0, 1)) * -1
    tmp13 = einsum(tmp12, (0, 1, 2, 3), t2, (0, 1, 4, 2), (4, 3)) * 0.5
    rdm2.ovov += einsum(tmp13, (0, 1), delta.oo, (2, 3), (2, 1, 3, 0)) * 8
    tmp19 = einsum(l2, (0, 1, 2, 3), t2, (4, 2, 1, 5), (3, 4, 0, 5))
    rdm2.ovov += np.transpose(tmp19, (1, 2, 0, 3)) * -2
    tmp3 = np.copy(np.transpose(l2, (3, 2, 0, 1))) * -1
    tmp3 += np.transpose(l2, (2, 3, 0, 1)) * 2
    tmp4 = einsum(tmp3, (0, 1, 2, 3), t2, (4, 0, 5, 2), (4, 1, 5, 3))
    del tmp3
    rdm2.ovvo = np.copy(np.transpose(tmp4, (0, 3, 2, 1))) * 4
    tmp6 = np.copy(np.transpose(l2, (3, 2, 0, 1))) * -1
    tmp6 += np.transpose(l2, (2, 3, 0, 1))
    tmp7 = einsum(t2, (0, 1, 2, 3), tmp6, (1, 4, 2, 5), (0, 4, 3, 5))
    del tmp6
    rdm2.ovvo += np.transpose(tmp7, (0, 3, 2, 1)) * -2
    rdm2.ovvo += einsum(tmp13, (0, 1), delta.oo, (2, 3), (2, 1, 0, 3)) * -4
    tmp17 = einsum(t2, (0, 1, 2, 3), l2, (4, 2, 5, 1), (5, 0, 4, 3))
    rdm2.ovvo += np.transpose(tmp17, (1, 2, 3, 0)) * -2
    rdm2.vovo = np.copy(np.transpose(tmp4, (3, 0, 2, 1))) * -2
    tmp26 = np.copy(np.transpose(l2, (3, 2, 0, 1)))
    tmp26 += np.transpose(l2, (2, 3, 0, 1)) * -1
    tmp27 = einsum(t2, (0, 1, 2, 3), tmp26, (1, 4, 2, 5), (0, 4, 3, 5))
    del tmp26
    rdm2.vovo += np.transpose(tmp27, (3, 0, 2, 1)) * -2
    del tmp27
    tmp25 = einsum(tmp12, (0, 1, 2, 3), t2, (0, 1, 4, 2), (4, 3))
    del tmp12
    rdm2.vovo += einsum(tmp25, (0, 1), delta.oo, (2, 3), (1, 2, 0, 3)) * 4
    rdm2.vovo += np.transpose(tmp19, (2, 1, 3, 0)) * -2
    rdm2.voov = np.copy(np.transpose(tmp24, (2, 1, 0, 3))) * 2
    del tmp24
    rdm2.voov += np.transpose(tmp22, (2, 1, 0, 3)) * -2
    del tmp22
    rdm2.voov += einsum(tmp25, (0, 1), delta.oo, (2, 3), (1, 2, 3, 0)) * -2
    del delta, tmp25
    rdm2.voov += np.transpose(tmp17, (2, 1, 0, 3)) * -2
    rdm2.voov += np.transpose(tmp4, (3, 0, 1, 2)) * 2
    tmp5 = einsum(tmp4, (0, 1, 2, 3), t2, (4, 1, 5, 3), (4, 0, 5, 2))
    tmp9 = np.copy(tmp5) * 2
    tmp8 = einsum(tmp7, (0, 1, 2, 3), t2, (4, 1, 3, 5), (4, 0, 5, 2))
    del tmp7
    tmp9 += np.transpose(tmp8, (1, 0, 3, 2))
    del tmp8
    rdm2.oovv = np.copy(tmp9) * 2
    rdm2.oovv += np.transpose(tmp9, (0, 1, 3, 2)) * -2
    del tmp9
    tmp10 = einsum(tmp4, (0, 1, 2, 3), t2, (4, 1, 3, 5), (4, 0, 5, 2))
    del tmp4
    rdm2.oovv += np.transpose(tmp10, (1, 0, 3, 2)) * -4
    rdm2.oovv += np.transpose(tmp10, (1, 0, 2, 3)) * 2
    rdm2.oovv += np.transpose(tmp10, (0, 1, 3, 2)) * 2
    rdm2.oovv += tmp10 * -4
    del tmp10
    tmp11 = einsum(tmp0, (0, 1, 2, 3), t2, (1, 0, 4, 5), (2, 3, 5, 4))
    del tmp0
    rdm2.oovv += tmp11 * 4
    rdm2.oovv += np.transpose(tmp11, (0, 1, 3, 2)) * -2
    del tmp11
    tmp14 = einsum(tmp13, (0, 1), t2, (2, 3, 1, 4), (2, 3, 4, 0)) * 2
    del tmp13
    tmp16 = np.copy(np.transpose(tmp14, (1, 0, 2, 3)))
    del tmp14
    tmp15 = einsum(t2, (0, 1, 2, 3), tmp2, (1, 4), (0, 4, 3, 2))
    del tmp2
    tmp16 += np.transpose(tmp15, (0, 1, 3, 2))
    del tmp15
    rdm2.oovv += tmp16 * -4
    rdm2.oovv += np.transpose(tmp16, (0, 1, 3, 2)) * 2
    rdm2.oovv += np.transpose(tmp16, (1, 0, 2, 3)) * 2
    rdm2.oovv += np.transpose(tmp16, (1, 0, 3, 2)) * -4
    del tmp16
    rdm2.oovv += np.transpose(t2, (0, 1, 3, 2)) * -2
    rdm2.oovv += t2 * 4
    tmp18 = einsum(tmp17, (0, 1, 2, 3), t2, (4, 0, 2, 5), (4, 1, 5, 3))
    del tmp17
    rdm2.oovv += tmp18 * 2
    del tmp18
    tmp20 = einsum(t2, (0, 1, 2, 3), tmp19, (1, 4, 2, 5), (0, 4, 3, 5))
    del tmp19
    rdm2.oovv += np.transpose(tmp20, (0, 1, 3, 2)) * 2
    del tmp20
    rdm2.oovv += tmp5 * 4
    del tmp5
    rdm2.ooov = np.zeros((t2.shape[0], t2.shape[0], t2.shape[0], t2.shape[-1]))
    rdm2.oovo = np.zeros((t2.shape[0], t2.shape[0], t2.shape[-1], t2.shape[0]))
    rdm2.ovoo = np.zeros((t2.shape[0], t2.shape[-1], t2.shape[0], t2.shape[0]))
    rdm2.vooo = np.zeros((t2.shape[-1], t2.shape[0], t2.shape[0], t2.shape[0]))
    rdm2.ovvv = np.zeros((t2.shape[0], t2.shape[-1], t2.shape[-1], t2.shape[-1]))
    rdm2.vovv = np.zeros((t2.shape[-1], t2.shape[0], t2.shape[-1], t2.shape[-1]))
    rdm2.vvov = np.zeros((t2.shape[-1], t2.shape[-1], t2.shape[0], t2.shape[-1]))
    rdm2.vvvo = np.zeros((t2.shape[-1], t2.shape[-1], t2.shape[-1], t2.shape[0]))
    rdm2 = pack_2e(rdm2.oooo, rdm2.ooov, rdm2.oovo, rdm2.ovoo, rdm2.vooo, rdm2.oovv, rdm2.ovov, rdm2.ovvo, rdm2.voov, rdm2.vovo, rdm2.vvoo, rdm2.ovvv, rdm2.vovv, rdm2.vvov, rdm2.vvvo, rdm2.vvvv)
    rdm2 = np.transpose(rdm2, (0, 2, 1, 3))

    return rdm2

def hbar_matvec_ip_intermediates(f=None, t2=None, v=None, **kwargs):
    """
    Code generated by `albert` 0.0.0 on 2024-10-18T21:56:19.198831.

    Parameters
    ----------
    f : array
        Fock matrix.
    t2 : array
        T2 amplitudes.
    v : array
        Electron repulsion integrals.

    Returns
    -------
    tmp0 : array
    tmp13 : array
    tmp15 : array
    tmp16 : array
    tmp18 : array
    tmp23 : array
    tmp24 : array
    tmp25 : array
    tmp27 : array
    tmp28 : array
    tmp3 : array
    tmp30 : array
    tmp31 : array
    tmp32 : array
    tmp37 : array
    tmp38 : array
    tmp39 : array
    tmp4 : array
    tmp40 : array
    tmp41 : array
    tmp43 : array
    tmp49 : array
    tmp50 : array
    tmp6 : array
    tmp7 : array
    """

    tmp18 = einsum(v.xvv, (0, 1, 2), v.xoo, (0, 3, 4), (3, 4, 1, 2))
    tmp23 = einsum(f.ov, (0, 1), t2, (2, 0, 1, 3), (2, 3))
    tmp25 = einsum(v.xoo, (0, 1, 2), v.xoo, (0, 3, 4), (3, 1, 2, 4))
    tmp3 = einsum(v.xov, (0, 1, 2), t2, (3, 1, 2, 4), (3, 4, 0))
    tmp4 = einsum(v.xov, (0, 1, 2), tmp3, (3, 2, 0), (3, 1))
    tmp24 = einsum(f.ov, (0, 1), t2, (2, 0, 3, 1), (2, 3))
    tmp30 = einsum(v.xoo, (0, 1, 2), tmp3, (3, 4, 0), (3, 1, 2, 4))
    tmp37 = einsum(f.ov, (0, 1), t2, (2, 3, 4, 1), (0, 2, 3, 4))
    tmp41 = einsum(v.xov, (0, 1, 2), tmp3, (1, 3, 0), (3, 2))
    tmp50 = einsum(v.xvv, (0, 1, 2), tmp3, (3, 2, 0), (3, 1))
    tmp6 = einsum(t2, (0, 1, 2, 3), v.xov, (4, 1, 3), (0, 2, 4))
    tmp7 = einsum(tmp6, (0, 1, 2), v.xov, (2, 3, 1), (0, 3))
    tmp12 = einsum(v.xvv, (0, 1, 2), v.xov, (0, 3, 4), (3, 1, 2, 4))
    tmp13 = einsum(t2, (0, 1, 2, 3), tmp12, (4, 5, 3, 2), (0, 1, 4, 5))
    del tmp12
    tmp15 = einsum(v.xov, (0, 1, 2), v.xov, (0, 3, 4), (3, 1, 4, 2))
    tmp16 = einsum(t2, (0, 1, 2, 3), tmp15, (4, 5, 2, 3), (0, 1, 4, 5))
    tmp31 = einsum(t2, (0, 1, 2, 3), tmp15, (1, 4, 5, 2), (0, 4, 3, 5))
    tmp32 = einsum(tmp15, (0, 1, 2, 3), t2, (4, 0, 5, 3), (4, 1, 5, 2))
    tmp43 = einsum(v.xov, (0, 1, 2), tmp6, (1, 3, 0), (3, 2))
    tmp0 = einsum(v.xov, (0, 1, 2), v.xoo, (0, 3, 4), (3, 4, 1, 2))
    tmp27 = einsum(tmp0, (0, 1, 2, 3), t2, (4, 1, 5, 3), (4, 0, 2, 5))
    tmp28 = einsum(tmp6, (0, 1, 2), v.xoo, (2, 3, 4), (0, 3, 4, 1))
    tmp38 = einsum(t2, (0, 1, 2, 3), tmp0, (4, 0, 1, 2), (4, 3))
    tmp39 = einsum(t2, (0, 1, 2, 3), tmp0, (4, 0, 1, 3), (4, 2))
    tmp40 = einsum(tmp0, (0, 1, 2, 3), t2, (4, 1, 3, 5), (4, 0, 2, 5))
    tmp49 = einsum(v.xvv, (0, 1, 2), tmp6, (3, 2, 0), (3, 1))

    return {f"tmp0": tmp0, f"tmp13": tmp13, f"tmp15": tmp15, f"tmp16": tmp16, f"tmp18": tmp18, f"tmp23": tmp23, f"tmp24": tmp24, f"tmp25": tmp25, f"tmp27": tmp27, f"tmp28": tmp28, f"tmp3": tmp3, f"tmp30": tmp30, f"tmp31": tmp31, f"tmp32": tmp32, f"tmp37": tmp37, f"tmp38": tmp38, f"tmp39": tmp39, f"tmp4": tmp4, f"tmp40": tmp40, f"tmp41": tmp41, f"tmp43": tmp43, f"tmp49": tmp49, f"tmp50": tmp50, f"tmp6": tmp6, f"tmp7": tmp7}

def hbar_matvec_ip(f=None, r1=None, r2=None, t2=None, v=None, **kwargs):
    """
    Code generated by `albert` 0.0.0 on 2024-10-18T21:56:19.907346.

    Parameters
    ----------
    f : array
        Fock matrix.
    r1 : array
        R1 amplitudes.
    r2 : array
        R2 amplitudes.
    t2 : array
        T2 amplitudes.
    v : array
        Electron repulsion integrals.

    Returns
    -------
    r1new : array
        Updated R1 residuals.
    r2new : array
        Updated R2 residuals.
    """

    ints = kwargs["ints"]
    tmp0 = np.copy(r2) * -1
    tmp0 += np.transpose(r2, (1, 0, 2)) * 2
    r1new = einsum(tmp0, (0, 1, 2), ints.tmp0, (3, 0, 1, 2), (3,))
    del tmp0
    tmp1 = np.copy(r2) * 2
    tmp1 += np.transpose(r2, (1, 0, 2)) * -1
    r1new += einsum(f.ov, (0, 1), tmp1, (0, 2, 1), (2,)) * -1
    tmp2 = np.copy(f.oo)
    tmp2 += np.transpose(ints.tmp7, (1, 0)) * 2
    del ints.tmp7
    tmp2 += np.transpose(ints.tmp4, (1, 0)) * -1
    del ints.tmp4
    r1new += einsum(tmp2, (0, 1), r1, (0,), (1,)) * -1
    r2new = einsum(ints.tmp32, (0, 1, 2, 3), r2, (4, 1, 3), (0, 4, 2))
    r2new += einsum(r2, (0, 1, 2), ints.tmp25, (3, 0, 4, 1), (4, 3, 2))
    del ints.tmp25
    r2new += einsum(r2, (0, 1, 2), ints.tmp16, (3, 4, 1, 0), (4, 3, 2))
    del ints.tmp16
    tmp3 = np.copy(ints.tmp18) * 0.5
    tmp3 += ints.tmp32
    del ints.tmp32
    tmp3 += ints.tmp31 * -0.5
    r2new += einsum(tmp3, (0, 1, 2, 3), r2, (1, 4, 3), (0, 4, 2)) * -2
    del tmp3
    tmp4 = np.copy(ints.tmp18)
    del ints.tmp18
    tmp4 += ints.tmp31 * -1
    del ints.tmp31
    r2new += einsum(tmp4, (0, 1, 2, 3), r2, (4, 1, 3), (4, 0, 2)) * -1
    del tmp4
    tmp6 = np.copy(ints.tmp6) * 2
    del ints.tmp6
    tmp6 += np.transpose(v.xov, (1, 2, 0))
    tmp6 += ints.tmp3 * -1
    del ints.tmp3
    tmp5 = einsum(tmp1, (0, 1, 2), v.xov, (3, 0, 2), (1, 3)) * 0.5
    r2new += einsum(tmp6, (0, 1, 2), tmp5, (3, 2), (0, 3, 1)) * 2
    del tmp6, tmp5
    tmp7 = np.copy(f.vv)
    tmp7 += np.transpose(ints.tmp41, (1, 0))
    del ints.tmp41
    tmp7 += np.transpose(ints.tmp43, (1, 0)) * -2
    del ints.tmp43
    r2new += einsum(r2, (0, 1, 2), tmp7, (2, 3), (0, 1, 3))
    del tmp7
    tmp8 = einsum(ints.tmp15, (0, 1, 2, 3), tmp1, (0, 1, 2), (3,)) * 0.5
    del tmp1, ints.tmp15
    r2new += einsum(t2, (0, 1, 2, 3), tmp8, (3,), (0, 1, 2)) * -2
    del tmp8
    tmp9 = np.copy(ints.tmp0)
    del ints.tmp0
    tmp9 += np.transpose(ints.tmp13, (0, 2, 1, 3))
    del ints.tmp13
    tmp9 += np.transpose(ints.tmp28, (2, 1, 0, 3)) * 2
    del ints.tmp28
    tmp9 += np.transpose(ints.tmp37, (2, 0, 1, 3))
    del ints.tmp37
    tmp9 += np.transpose(ints.tmp27, (1, 2, 0, 3)) * -1
    del ints.tmp27
    tmp9 += np.transpose(ints.tmp30, (2, 1, 0, 3)) * -1
    del ints.tmp30
    tmp9 += np.transpose(ints.tmp40, (0, 2, 1, 3)) * -1
    del ints.tmp40
    r2new += einsum(tmp9, (0, 1, 2, 3), r1, (1,), (2, 0, 3))
    del tmp9
    r2new += einsum(r2, (0, 1, 2), tmp2, (0, 3), (3, 1, 2)) * -1
    r2new += einsum(tmp2, (0, 1), r2, (2, 0, 3), (2, 1, 3)) * -1
    del tmp2
    tmp10 = np.copy(f.ov)
    tmp10 += ints.tmp24 * 2
    del ints.tmp24
    tmp10 += ints.tmp38
    del ints.tmp38
    tmp10 += ints.tmp49 * 2
    del ints.tmp49
    tmp10 += ints.tmp23 * -1
    del ints.tmp23
    tmp10 += ints.tmp39 * -2
    del ints.tmp39
    tmp10 += ints.tmp50 * -1
    del ints.tmp50
    r2new += einsum(tmp10, (0, 1), r1, (2,), (0, 2, 1)) * -1
    del tmp10

    return {f"r1new": r1new, f"r2new": r2new}

def hbar_matvec_ea_intermediates(f=None, t2=None, v=None, **kwargs):
    """
    Code generated by `albert` 0.0.0 on 2024-10-18T21:58:39.871907.

    Parameters
    ----------
    f : array
        Fock matrix.
    t2 : array
        T2 amplitudes.
    v : array
        Electron repulsion integrals.

    Returns
    -------
    tmp0 : array
    tmp12 : array
    tmp15 : array
    tmp16 : array
    tmp18 : array
    tmp20 : array
    tmp28 : array
    tmp3 : array
    tmp30 : array
    tmp31 : array
    tmp32 : array
    tmp34 : array
    tmp36 : array
    tmp37 : array
    tmp38 : array
    tmp4 : array
    tmp40 : array
    tmp41 : array
    tmp44 : array
    tmp49 : array
    tmp50 : array
    tmp6 : array
    tmp7 : array
    """

    tmp18 = einsum(v.xvv, (0, 1, 2), v.xoo, (0, 3, 4), (3, 4, 1, 2))
    tmp28 = einsum(v.xvv, (0, 1, 2), v.xvv, (0, 3, 4), (1, 3, 4, 2))
    tmp36 = einsum(f.ov, (0, 1), t2, (2, 0, 1, 3), (2, 3))
    tmp3 = einsum(v.xov, (0, 1, 2), t2, (3, 1, 2, 4), (3, 4, 0))
    tmp4 = einsum(v.xov, (0, 1, 2), tmp3, (1, 3, 0), (3, 2))
    tmp31 = einsum(v.xvv, (0, 1, 2), tmp3, (3, 2, 0), (3, 1))
    tmp32 = einsum(v.xov, (0, 1, 2), tmp3, (3, 2, 0), (3, 1))
    tmp37 = einsum(f.ov, (0, 1), t2, (2, 0, 3, 1), (2, 3))
    tmp41 = einsum(v.xvv, (0, 1, 2), tmp3, (3, 4, 0), (3, 4, 1, 2))
    tmp6 = einsum(v.xov, (0, 1, 2), t2, (3, 1, 4, 2), (3, 4, 0))
    tmp7 = einsum(v.xov, (0, 1, 2), tmp6, (1, 3, 0), (3, 2))
    tmp0 = einsum(v.xvv, (0, 1, 2), v.xov, (0, 3, 4), (3, 1, 2, 4))
    tmp12 = einsum(tmp0, (0, 1, 2, 3), t2, (4, 0, 2, 5), (4, 5, 3, 1))
    tmp15 = einsum(v.xov, (0, 1, 2), v.xov, (0, 3, 4), (3, 1, 4, 2))
    tmp16 = einsum(t2, (0, 1, 2, 3), tmp15, (1, 4, 5, 2), (0, 4, 3, 5))
    tmp34 = einsum(tmp6, (0, 1, 2), v.xov, (2, 3, 1), (0, 3))
    tmp40 = einsum(tmp0, (0, 1, 2, 3), t2, (4, 0, 5, 2), (4, 5, 3, 1))
    tmp44 = einsum(tmp15, (0, 1, 2, 3), t2, (4, 0, 5, 3), (4, 1, 5, 2))
    tmp19 = einsum(v.xov, (0, 1, 2), v.xoo, (0, 3, 4), (3, 4, 1, 2))
    tmp20 = einsum(t2, (0, 1, 2, 3), tmp19, (4, 0, 1, 5), (4, 2, 3, 5))
    tmp30 = einsum(v.xvv, (0, 1, 2), tmp6, (3, 2, 0), (3, 1))
    tmp38 = einsum(tmp6, (0, 1, 2), v.xvv, (2, 3, 4), (0, 1, 3, 4))
    tmp49 = einsum(t2, (0, 1, 2, 3), tmp19, (4, 0, 1, 2), (4, 3))
    tmp50 = einsum(t2, (0, 1, 2, 3), tmp19, (4, 0, 1, 3), (4, 2))
    del tmp19

    return {f"tmp0": tmp0, f"tmp12": tmp12, f"tmp15": tmp15, f"tmp16": tmp16, f"tmp18": tmp18, f"tmp20": tmp20, f"tmp28": tmp28, f"tmp3": tmp3, f"tmp30": tmp30, f"tmp31": tmp31, f"tmp32": tmp32, f"tmp34": tmp34, f"tmp36": tmp36, f"tmp37": tmp37, f"tmp38": tmp38, f"tmp4": tmp4, f"tmp40": tmp40, f"tmp41": tmp41, f"tmp44": tmp44, f"tmp49": tmp49, f"tmp50": tmp50, f"tmp6": tmp6, f"tmp7": tmp7}

def hbar_matvec_ea(f=None, r1=None, r2=None, t2=None, v=None, **kwargs):
    """
    Code generated by `albert` 0.0.0 on 2024-10-18T21:58:40.513506.

    Parameters
    ----------
    f : array
        Fock matrix.
    r1 : array
        R1 amplitudes.
    r2 : array
        R2 amplitudes.
    t2 : array
        T2 amplitudes.
    v : array
        Electron repulsion integrals.

    Returns
    -------
    r1new : array
        Updated R1 residuals.
    r2new : array
        Updated R2 residuals.
    """

    ints = kwargs["ints"]
    tmp0 = np.copy(np.transpose(r2, (2, 0, 1))) * -0.5
    tmp0 += np.transpose(r2, (2, 1, 0))
    r1new = einsum(ints.tmp0, (0, 1, 2, 3), tmp0, (0, 2, 3), (1,)) * -2
    del tmp0
    tmp1 = np.copy(np.transpose(r2, (2, 0, 1))) * -1
    tmp1 += np.transpose(r2, (2, 1, 0)) * 2
    r1new += einsum(f.ov, (0, 1), tmp1, (0, 2, 1), (2,)) * -1
    tmp2 = np.copy(f.vv)
    tmp2 += np.transpose(ints.tmp4, (1, 0))
    del ints.tmp4
    tmp2 += np.transpose(ints.tmp7, (1, 0)) * -2
    del ints.tmp7
    r1new += einsum(tmp2, (0, 1), r1, (0,), (1,))
    tmp3 = einsum(r2, (0, 1, 2), ints.tmp15, (3, 4, 1, 0), (3, 4, 2))
    r2new = einsum(t2, (0, 1, 2, 3), tmp3, (1, 0, 4), (2, 3, 4))
    del tmp3
    r2new += einsum(r2, (0, 1, 2), ints.tmp28, (3, 0, 4, 1), (4, 3, 2))
    del ints.tmp28
    r2new += einsum(ints.tmp44, (0, 1, 2, 3), r2, (4, 3, 1), (2, 4, 0))
    tmp4 = np.copy(ints.tmp18) * 0.5
    tmp4 += ints.tmp44
    del ints.tmp44
    tmp4 += ints.tmp16 * -0.5
    r2new += einsum(tmp4, (0, 1, 2, 3), r2, (3, 4, 1), (2, 4, 0)) * -2
    del tmp4
    tmp5 = np.copy(ints.tmp16)
    del ints.tmp16
    tmp5 += ints.tmp18 * -1
    del ints.tmp18
    r2new += einsum(r2, (0, 1, 2), tmp5, (3, 2, 4, 1), (0, 4, 3))
    del tmp5
    tmp7 = np.copy(ints.tmp6) * 2
    del ints.tmp6
    tmp7 += np.transpose(v.xov, (1, 2, 0))
    tmp7 += ints.tmp3 * -1
    del ints.tmp3
    tmp6 = einsum(v.xov, (0, 1, 2), tmp1, (1, 3, 2), (3, 0)) * 0.5
    del tmp1
    r2new += einsum(tmp7, (0, 1, 2), tmp6, (3, 2), (1, 3, 0)) * 2
    del tmp7, tmp6
    tmp8 = np.copy(ints.tmp0)
    del ints.tmp0
    tmp8 += np.transpose(ints.tmp20, (0, 2, 3, 1))
    del ints.tmp20
    tmp8 += np.transpose(ints.tmp38, (0, 3, 2, 1)) * 2
    del ints.tmp38
    tmp8 += ints.tmp12 * -1
    del ints.tmp12
    tmp8 += np.transpose(ints.tmp40, (0, 3, 2, 1)) * -1
    del ints.tmp40
    tmp8 += np.transpose(ints.tmp41, (0, 3, 2, 1)) * -1
    del ints.tmp41
    r2new += einsum(tmp8, (0, 1, 2, 3), r1, (2,), (3, 1, 0)) * -1
    del tmp8
    r2new += einsum(tmp2, (0, 1), r2, (0, 2, 3), (1, 2, 3))
    r2new += einsum(tmp2, (0, 1), r2, (2, 0, 3), (2, 1, 3))
    del tmp2
    tmp10 = einsum(f.ov, (0, 1), r1, (1,), (0,)) * 0.5
    tmp9 = np.copy(ints.tmp15) * 2
    tmp9 += np.transpose(ints.tmp15, (0, 1, 3, 2)) * -1
    del ints.tmp15
    tmp10 += einsum(tmp9, (0, 1, 2, 3), r2, (2, 3, 0), (1,)) * -0.5
    del tmp9
    r2new += einsum(t2, (0, 1, 2, 3), tmp10, (1,), (2, 3, 0)) * 2
    del tmp10
    tmp11 = np.copy(f.oo)
    tmp11 += np.transpose(ints.tmp34, (1, 0)) * 2
    del ints.tmp34
    tmp11 += np.transpose(ints.tmp32, (1, 0)) * -1
    del ints.tmp32
    r2new += einsum(tmp11, (0, 1), r2, (2, 3, 0), (2, 3, 1)) * -1
    del tmp11
    tmp12 = np.copy(f.ov)
    tmp12 += ints.tmp30 * 2
    del ints.tmp30
    tmp12 += ints.tmp37 * 2
    del ints.tmp37
    tmp12 += ints.tmp49
    del ints.tmp49
    tmp12 += ints.tmp31 * -1
    del ints.tmp31
    tmp12 += ints.tmp36 * -1
    del ints.tmp36
    tmp12 += ints.tmp50 * -2
    del ints.tmp50
    r2new += einsum(tmp12, (0, 1), r1, (2,), (1, 2, 0)) * -1
    del tmp12

    return {f"r1new": r1new, f"r2new": r2new}

def hbar_lmatvec_ip_intermediates(f=None, t2=None, v=None, **kwargs):
    """
    Code generated by `albert` 0.0.0 on 2024-10-18T22:01:43.045933.

    Parameters
    ----------
    f : array
        Fock matrix.
    t2 : array
        T2 amplitudes.
    v : array
        Electron repulsion integrals.

    Returns
    -------
    tmp0 : array
    tmp1 : array
    tmp10 : array
    tmp12 : array
    tmp13 : array
    tmp15 : array
    tmp18 : array
    tmp21 : array
    tmp23 : array
    tmp25 : array
    tmp27 : array
    tmp29 : array
    tmp3 : array
    tmp32 : array
    tmp36 : array
    tmp41 : array
    tmp43 : array
    tmp44 : array
    tmp45 : array
    tmp59 : array
    tmp6 : array
    tmp63 : array
    tmp65 : array
    tmp67 : array
    tmp9 : array
    """

    tmp0 = einsum(f.ov, (0, 1), t2, (2, 0, 1, 3), (2, 3))
    tmp59 = einsum(v.xoo, (0, 1, 2), v.xoo, (0, 3, 4), (3, 1, 2, 4))
    tmp1 = einsum(f.ov, (0, 1), t2, (2, 0, 3, 1), (2, 3))
    tmp3 = einsum(f.ov, (0, 1), t2, (2, 3, 4, 1), (0, 2, 3, 4))
    tmp41 = einsum(v.xvv, (0, 1, 2), v.xoo, (0, 3, 4), (3, 4, 1, 2))
    tmp9 = einsum(v.xov, (0, 1, 2), t2, (3, 1, 2, 4), (3, 4, 0))
    tmp10 = einsum(v.xov, (0, 1, 2), tmp9, (3, 2, 0), (3, 1))
    tmp12 = einsum(v.xov, (0, 1, 2), t2, (3, 1, 4, 2), (3, 4, 0))
    tmp13 = einsum(tmp12, (0, 1, 2), v.xov, (2, 3, 1), (0, 3))
    tmp6 = einsum(v.xov, (0, 1, 2), v.xoo, (0, 3, 4), (3, 4, 1, 2))
    tmp15 = einsum(t2, (0, 1, 2, 3), tmp6, (4, 0, 1, 2), (4, 3))
    tmp18 = einsum(t2, (0, 1, 2, 3), tmp6, (4, 0, 1, 3), (4, 2))
    tmp21 = einsum(t2, (0, 1, 2, 3), tmp6, (1, 4, 5, 2), (0, 4, 5, 3))
    tmp23 = einsum(t2, (0, 1, 2, 3), tmp6, (1, 4, 5, 3), (0, 4, 5, 2))
    tmp29 = einsum(v.xvv, (0, 1, 2), tmp12, (3, 2, 0), (3, 1))
    tmp43 = einsum(v.xov, (0, 1, 2), v.xov, (0, 3, 4), (3, 1, 4, 2))
    tmp44 = einsum(t2, (0, 1, 2, 3), tmp43, (4, 1, 2, 5), (0, 4, 3, 5))
    tmp45 = einsum(tmp43, (0, 1, 2, 3), t2, (4, 1, 5, 2), (4, 0, 5, 3))
    tmp63 = einsum(tmp43, (0, 1, 2, 3), t2, (4, 5, 2, 3), (4, 5, 0, 1))
    tmp65 = einsum(v.xov, (0, 1, 2), tmp9, (1, 3, 0), (3, 2))
    tmp67 = einsum(tmp12, (0, 1, 2), v.xov, (2, 0, 3), (1, 3))
    tmp25 = einsum(v.xoo, (0, 1, 2), tmp9, (3, 4, 0), (3, 1, 2, 4))
    tmp27 = einsum(tmp12, (0, 1, 2), v.xoo, (2, 3, 4), (0, 3, 4, 1))
    tmp32 = einsum(v.xvv, (0, 1, 2), tmp9, (3, 2, 0), (3, 1))
    tmp35 = einsum(v.xvv, (0, 1, 2), v.xov, (0, 3, 4), (3, 1, 2, 4))
    tmp36 = einsum(t2, (0, 1, 2, 3), tmp35, (4, 5, 3, 2), (0, 1, 4, 5))
    del tmp35

    return {f"tmp0": tmp0, f"tmp1": tmp1, f"tmp10": tmp10, f"tmp12": tmp12, f"tmp13": tmp13, f"tmp15": tmp15, f"tmp18": tmp18, f"tmp21": tmp21, f"tmp23": tmp23, f"tmp25": tmp25, f"tmp27": tmp27, f"tmp29": tmp29, f"tmp3": tmp3, f"tmp32": tmp32, f"tmp36": tmp36, f"tmp41": tmp41, f"tmp43": tmp43, f"tmp44": tmp44, f"tmp45": tmp45, f"tmp59": tmp59, f"tmp6": tmp6, f"tmp63": tmp63, f"tmp65": tmp65, f"tmp67": tmp67, f"tmp9": tmp9}

def hbar_lmatvec_ip(f=None, r1=None, r2=None, t2=None, v=None, **kwargs):
    """
    Code generated by `albert` 0.0.0 on 2024-10-18T22:01:43.626269.

    Parameters
    ----------
    f : array
        Fock matrix.
    r1 : array
        R1 amplitudes.
    r2 : array
        R2 amplitudes.
    t2 : array
        T2 amplitudes.
    v : array
        Electron repulsion integrals.

    Returns
    -------
    r1new : array
        Updated R1 residuals.
    r2new : array
        Updated R2 residuals.
    """

    ints = kwargs["ints"]
    tmp1 = np.copy(ints.tmp36)
    del ints.tmp36
    tmp1 += np.transpose(ints.tmp6, (1, 2, 0, 3))
    tmp1 += ints.tmp21 * -1
    del ints.tmp21
    tmp0 = np.copy(r2) * -0.5
    tmp0 += np.transpose(r2, (1, 0, 2))
    r1new = einsum(tmp0, (0, 1, 2), tmp1, (0, 1, 3, 2), (3,)) * 2
    del tmp0, tmp1
    tmp3 = np.copy(ints.tmp27) * 2
    del ints.tmp27
    tmp3 += np.transpose(ints.tmp3, (1, 2, 0, 3))
    del ints.tmp3
    tmp3 += ints.tmp23 * -1
    del ints.tmp23
    tmp3 += ints.tmp25 * -1
    del ints.tmp25
    tmp2 = np.copy(r2) * 2
    tmp2 += np.transpose(r2, (1, 0, 2)) * -1
    r1new += einsum(tmp3, (0, 1, 2, 3), tmp2, (0, 1, 3), (2,))
    del tmp3
    tmp4 = np.copy(f.ov)
    tmp4 += ints.tmp15
    del ints.tmp15
    tmp4 += ints.tmp1 * 2
    del ints.tmp1
    tmp4 += ints.tmp29 * 2
    del ints.tmp29
    tmp4 += ints.tmp0 * -1
    del ints.tmp0
    tmp4 += ints.tmp18 * -2
    del ints.tmp18
    tmp4 += ints.tmp32 * -1
    del ints.tmp32
    r1new += einsum(tmp2, (0, 1, 2), tmp4, (0, 2), (1,)) * -1
    del tmp4
    tmp5 = np.copy(f.oo)
    tmp5 += ints.tmp13 * 2
    del ints.tmp13
    tmp5 += ints.tmp10 * -1
    del ints.tmp10
    r1new += einsum(tmp5, (0, 1), r1, (0,), (1,)) * -1
    r2new = einsum(r2, (0, 1, 2), ints.tmp45, (1, 3, 2, 4), (3, 0, 4))
    r2new += einsum(f.ov, (0, 1), r1, (2,), (0, 2, 1)) * -1
    r2new += einsum(ints.tmp6, (0, 1, 2, 3), r1, (1,), (2, 0, 3))
    del ints.tmp6
    r2new += einsum(ints.tmp59, (0, 1, 2, 3), r2, (1, 3, 4), (2, 0, 4))
    del ints.tmp59
    r2new += einsum(r2, (0, 1, 2), ints.tmp63, (1, 0, 3, 4), (4, 3, 2))
    del ints.tmp63
    tmp6 = np.copy(ints.tmp41) * 0.5
    tmp6 += np.transpose(ints.tmp45, (1, 0, 3, 2))
    del ints.tmp45
    tmp6 += np.transpose(ints.tmp44, (1, 0, 3, 2)) * -0.5
    r2new += einsum(tmp6, (0, 1, 2, 3), r2, (1, 4, 3), (0, 4, 2)) * -2
    del tmp6
    tmp7 = np.copy(ints.tmp41)
    del ints.tmp41
    tmp7 += np.transpose(ints.tmp44, (1, 0, 3, 2)) * -1
    del ints.tmp44
    r2new += einsum(tmp7, (0, 1, 2, 3), r2, (4, 1, 3), (4, 0, 2)) * -1
    del tmp7
    tmp8 = np.copy(ints.tmp12)
    del ints.tmp12
    tmp8 += np.transpose(v.xov, (1, 2, 0)) * 0.5
    tmp8 += ints.tmp9 * -0.5
    del ints.tmp9
    tmp9 = einsum(tmp2, (0, 1, 2), tmp8, (0, 2, 3), (1, 3)) * 2
    del tmp2, tmp8
    r2new += einsum(tmp9, (0, 1), v.xov, (1, 2, 3), (2, 0, 3))
    del tmp9
    tmp10 = np.copy(f.vv)
    tmp10 += ints.tmp65
    del ints.tmp65
    tmp10 += ints.tmp67 * -2
    del ints.tmp67
    r2new += einsum(r2, (0, 1, 2), tmp10, (2, 3), (0, 1, 3))
    del tmp10
    tmp11 = np.copy(np.transpose(t2, (0, 1, 3, 2))) * -0.5
    tmp11 += t2
    tmp12 = einsum(tmp11, (0, 1, 2, 3), r2, (0, 1, 2), (3,))
    del tmp11
    r2new += einsum(tmp12, (0,), ints.tmp43, (1, 2, 3, 0), (1, 2, 3)) * -2
    del tmp12, ints.tmp43
    r2new += einsum(r2, (0, 1, 2), tmp5, (0, 3), (3, 1, 2)) * -1
    r2new += einsum(r2, (0, 1, 2), tmp5, (1, 3), (0, 3, 2)) * -1
    del tmp5

    return {f"r1new": r1new, f"r2new": r2new}

def hbar_lmatvec_ea_intermediates(f=None, t2=None, v=None, **kwargs):
    """
    Code generated by `albert` 0.0.0 on 2024-10-18T22:04:34.036208.

    Parameters
    ----------
    f : array
        Fock matrix.
    t2 : array
        T2 amplitudes.
    v : array
        Electron repulsion integrals.

    Returns
    -------
    tmp0 : array
    tmp1 : array
    tmp10 : array
    tmp11 : array
    tmp13 : array
    tmp14 : array
    tmp17 : array
    tmp20 : array
    tmp23 : array
    tmp26 : array
    tmp29 : array
    tmp32 : array
    tmp34 : array
    tmp37 : array
    tmp39 : array
    tmp43 : array
    tmp45 : array
    tmp47 : array
    tmp49 : array
    tmp50 : array
    tmp51 : array
    tmp61 : array
    tmp7 : array
    """

    tmp0 = einsum(f.ov, (0, 1), t2, (2, 0, 1, 3), (2, 3))
    tmp61 = einsum(v.xvv, (0, 1, 2), v.xvv, (0, 3, 4), (1, 3, 4, 2))
    tmp1 = einsum(f.ov, (0, 1), t2, (2, 0, 3, 1), (2, 3))
    tmp43 = einsum(v.xvv, (0, 1, 2), v.xoo, (0, 3, 4), (3, 4, 1, 2))
    tmp10 = einsum(v.xov, (0, 1, 2), t2, (3, 1, 2, 4), (3, 4, 0))
    tmp11 = einsum(tmp10, (0, 1, 2), v.xov, (2, 0, 3), (1, 3))
    tmp13 = einsum(v.xov, (0, 1, 2), t2, (3, 1, 4, 2), (3, 4, 0))
    tmp14 = einsum(v.xov, (0, 1, 2), tmp13, (1, 3, 0), (3, 2))
    tmp16 = einsum(v.xov, (0, 1, 2), v.xoo, (0, 3, 4), (3, 4, 1, 2))
    tmp17 = einsum(tmp16, (0, 1, 2, 3), t2, (0, 2, 3, 4), (1, 4))
    tmp20 = einsum(tmp16, (0, 1, 2, 3), t2, (0, 2, 4, 3), (1, 4))
    tmp23 = einsum(t2, (0, 1, 2, 3), tmp16, (0, 4, 1, 5), (4, 2, 3, 5))
    del tmp16
    tmp26 = einsum(v.xvv, (0, 1, 2), tmp13, (3, 2, 0), (3, 1))
    tmp29 = einsum(v.xvv, (0, 1, 2), tmp10, (3, 2, 0), (3, 1))
    tmp32 = einsum(tmp10, (0, 1, 2), v.xvv, (2, 3, 4), (0, 1, 3, 4))
    tmp45 = einsum(tmp10, (0, 1, 2), v.xov, (2, 3, 1), (0, 3))
    tmp47 = einsum(v.xov, (0, 1, 2), tmp13, (3, 2, 0), (3, 1))
    tmp49 = einsum(v.xov, (0, 1, 2), v.xov, (0, 3, 4), (3, 1, 4, 2))
    tmp50 = einsum(t2, (0, 1, 2, 3), tmp49, (4, 1, 2, 5), (0, 4, 3, 5))
    tmp51 = einsum(t2, (0, 1, 2, 3), tmp49, (4, 1, 3, 5), (0, 4, 2, 5))
    tmp34 = einsum(v.xvv, (0, 1, 2), tmp13, (3, 4, 0), (3, 4, 1, 2))
    tmp7 = einsum(v.xvv, (0, 1, 2), v.xov, (0, 3, 4), (3, 1, 2, 4))
    tmp37 = einsum(tmp7, (0, 1, 2, 3), t2, (4, 0, 2, 5), (4, 5, 3, 1))
    tmp39 = einsum(tmp7, (0, 1, 2, 3), t2, (4, 0, 5, 2), (4, 5, 3, 1))

    return {f"tmp0": tmp0, f"tmp1": tmp1, f"tmp10": tmp10, f"tmp11": tmp11, f"tmp13": tmp13, f"tmp14": tmp14, f"tmp17": tmp17, f"tmp20": tmp20, f"tmp23": tmp23, f"tmp26": tmp26, f"tmp29": tmp29, f"tmp32": tmp32, f"tmp34": tmp34, f"tmp37": tmp37, f"tmp39": tmp39, f"tmp43": tmp43, f"tmp45": tmp45, f"tmp47": tmp47, f"tmp49": tmp49, f"tmp50": tmp50, f"tmp51": tmp51, f"tmp61": tmp61, f"tmp7": tmp7}

def hbar_lmatvec_ea(f=None, r1=None, r2=None, t2=None, v=None, **kwargs):
    """
    Code generated by `albert` 0.0.0 on 2024-10-18T22:04:34.572114.

    Parameters
    ----------
    f : array
        Fock matrix.
    r1 : array
        R1 amplitudes.
    r2 : array
        R2 amplitudes.
    t2 : array
        T2 amplitudes.
    v : array
        Electron repulsion integrals.

    Returns
    -------
    r1new : array
        Updated R1 residuals.
    r2new : array
        Updated R2 residuals.
    """

    ints = kwargs["ints"]
    tmp0 = np.copy(ints.tmp23) * 0.5
    tmp0 += ints.tmp34
    tmp0 += np.transpose(ints.tmp37, (0, 1, 3, 2))
    tmp0 += ints.tmp32 * -0.5
    tmp0 += np.transpose(ints.tmp39, (0, 1, 3, 2)) * -0.5
    tmp0 += np.transpose(ints.tmp7, (0, 2, 3, 1)) * -1
    r1new = einsum(r2, (0, 1, 2), tmp0, (2, 1, 0, 3), (3,)) * 2
    del tmp0
    tmp1 = np.copy(ints.tmp23) * 2
    del ints.tmp23
    tmp1 += ints.tmp34 * 4
    del ints.tmp34
    tmp1 += np.transpose(ints.tmp37, (0, 1, 3, 2))
    del ints.tmp37
    tmp1 += ints.tmp32 * -2
    del ints.tmp32
    tmp1 += np.transpose(ints.tmp39, (0, 1, 3, 2)) * -2
    del ints.tmp39
    tmp1 += np.transpose(ints.tmp7, (0, 2, 3, 1)) * -1
    r1new += einsum(r2, (0, 1, 2), tmp1, (2, 0, 1, 3), (3,)) * -1
    del tmp1
    tmp2 = np.copy(f.ov)
    tmp2 += ints.tmp17
    del ints.tmp17
    tmp2 += ints.tmp1 * 2
    del ints.tmp1
    tmp2 += ints.tmp26 * 2
    del ints.tmp26
    tmp2 += ints.tmp0 * -1
    del ints.tmp0
    tmp2 += ints.tmp20 * -2
    del ints.tmp20
    tmp2 += ints.tmp29 * -1
    del ints.tmp29
    tmp3 = np.copy(np.transpose(r2, (2, 0, 1))) * -1
    tmp3 += np.transpose(r2, (2, 1, 0)) * 2
    r1new += einsum(tmp2, (0, 1), tmp3, (0, 2, 1), (2,)) * -1
    del tmp2, tmp3
    tmp4 = np.copy(f.vv)
    tmp4 += ints.tmp11
    del ints.tmp11
    tmp4 += ints.tmp14 * -2
    del ints.tmp14
    r1new += einsum(tmp4, (0, 1), r1, (0,), (1,))
    tmp5 = np.copy(np.transpose(t2, (0, 1, 3, 2)))
    tmp5 += t2 * -0.5
    tmp6 = einsum(tmp5, (0, 1, 2, 3), r2, (3, 2, 0), (1,)) * 2
    r1new += einsum(f.ov, (0, 1), tmp6, (0,), (1,))
    del tmp6
    tmp7 = einsum(r2, (0, 1, 2), t2, (3, 4, 0, 1), (2, 3, 4))
    r2new = einsum(ints.tmp49, (0, 1, 2, 3), tmp7, (4, 1, 0), (3, 2, 4))
    del tmp7
    r2new += einsum(f.ov, (0, 1), r1, (2,), (1, 2, 0)) * -1
    r2new += einsum(r1, (0,), ints.tmp7, (1, 2, 0, 3), (3, 2, 1)) * -1
    del ints.tmp7
    r2new += einsum(r2, (0, 1, 2), ints.tmp61, (3, 0, 4, 1), (4, 3, 2))
    del ints.tmp61
    r2new += einsum(ints.tmp51, (0, 1, 2, 3), r2, (4, 2, 0), (3, 4, 1))
    tmp8 = np.copy(ints.tmp43) * 0.5
    tmp8 += np.transpose(ints.tmp51, (1, 0, 3, 2))
    del ints.tmp51
    tmp8 += np.transpose(ints.tmp50, (1, 0, 3, 2)) * -0.5
    r2new += einsum(tmp8, (0, 1, 2, 3), r2, (3, 4, 1), (2, 4, 0)) * -2
    del tmp8
    tmp9 = np.copy(ints.tmp43)
    del ints.tmp43
    tmp9 += np.transpose(ints.tmp50, (1, 0, 3, 2)) * -1
    del ints.tmp50
    r2new += einsum(r2, (0, 1, 2), tmp9, (3, 2, 4, 1), (0, 4, 3)) * -1
    del tmp9
    tmp11 = np.copy(ints.tmp13) * 2
    del ints.tmp13
    tmp11 += np.transpose(v.xov, (1, 2, 0))
    tmp11 += ints.tmp10 * -1
    del ints.tmp10
    tmp10 = np.copy(np.transpose(r2, (2, 0, 1))) * -0.5
    tmp10 += np.transpose(r2, (2, 1, 0))
    tmp12 = einsum(tmp11, (0, 1, 2), tmp10, (0, 3, 1), (3, 2)) * 2
    del tmp11, tmp10
    r2new += einsum(tmp12, (0, 1), v.xov, (1, 2, 3), (3, 0, 2))
    del tmp12
    r2new += einsum(r2, (0, 1, 2), tmp4, (0, 3), (3, 1, 2))
    r2new += einsum(tmp4, (0, 1), r2, (2, 0, 3), (2, 1, 3))
    del tmp4
    tmp13 = np.copy(f.oo)
    tmp13 += ints.tmp47 * 2
    del ints.tmp47
    tmp13 += ints.tmp45 * -1
    del ints.tmp45
    r2new += einsum(tmp13, (0, 1), r2, (2, 3, 0), (2, 3, 1)) * -1
    del tmp13
    tmp14 = einsum(tmp5, (0, 1, 2, 3), r2, (3, 2, 0), (1,))
    del tmp5
    r2new += einsum(ints.tmp49, (0, 1, 2, 3), tmp14, (1,), (2, 3, 0)) * -2
    del ints.tmp49, tmp14

    return {f"r1new": r1new, f"r2new": r2new}

