...
def update_amps(f=None, v=None, nocc=None, nvir=None, t1=None, t2=None, **kwargs):
    ...
    del x7
    x8 = np.zeros((nocc, nvir, nvir, nvir), dtype=types[float])
    x8 += einsum(v.ovvv, (0, 1, 2, 3), (0, 1, 2, 3))
    x8 += einsum(v.ovvv, (0, 1, 2, 3), (0, 2, 3, 1)) * -0.5
    x9 = np.zeros((nvir, nvir), dtype=types[float])
...
def energy_perturbative(f=None, v=None, nocc=None, nvir=None, t1=None, t2=None, l1=None, l2=None, **kwargs):
    ...
    del x10
    x11 = np.zeros((nocc, nocc, nocc, nvir, nvir), dtype=types[float])
    ...
    del x17
    x18 = np.zeros((nocc, nocc, nocc, nvir, nvir, nvir), dtype=types[float])
    ...
    del x22
    x23 = np.zeros((nocc, nocc, nocc, nvir, nvir, nvir), dtype=types[float])
    ...
    del x27
    x28 = np.zeros((nocc, nocc, nocc, nvir, nvir, nvir), dtype=types[float])
    ...
    del x34
    x35 = np.zeros((nocc, nocc, nocc, nvir, nvir, nvir), dtype=types[float])
    ...
    del x36
    x37 = np.zeros((nocc, nocc, nocc, nvir, nvir, nvir), dtype=types[float])
    ...
    del x38
    x39 = np.zeros((nocc, nocc, nocc, nvir, nvir), dtype=types[float])
    ...
    del x40
    x41 = np.zeros((nocc, nocc, nocc, nvir, nvir, nvir), dtype=types[float])
    ...
    del x43
    x44 = np.zeros((nocc, nocc, nocc, nvir, nvir, nvir), dtype=types[float])
    ...
    del x52
    x53 = np.zeros((nocc, nocc, nocc, nvir, nvir), dtype=types[float])
    ...
    del x54
    x55 = np.zeros((nocc, nocc, nocc, nocc, nvir, nvir), dtype=types[float])
    ...
    del x57
    x58 = np.zeros((nocc, nocc, nocc, nvir, nvir), dtype=types[float])
    ...
    del x59
    x60 = np.zeros((nocc, nocc, nocc, nocc, nvir), dtype=types[float])
    ...
    del x63
    x64 = np.zeros((nocc, nocc, nocc, nocc, nocc, nvir), dtype=types[float])
    ...
    del x66
    x67 = np.zeros((nocc, nocc, nocc, nocc, nvir, nvir), dtype=types[float])
    ...
    del x70
    x71 = np.zeros((nocc, nocc, nocc, nocc, nocc, nvir), dtype=types[float])
    ...
    del x76
    x77 = np.zeros((nocc, nocc, nocc, nocc, nvir, nvir), dtype=types[float])
    ...
    del x80
    x81 = np.zeros((nocc, nocc, nocc, nocc, nvir), dtype=types[float])
    x81 += einsum(l2, (0, 1, 2, 3), t2, (4, 2, 1, 0), denom3, (2, 5, 6, 1, 7, 0), (6, 5, 3, 4, 7))
    x82 = np.zeros((nocc, nocc, nocc, nvir), dtype=types[float])
    ...
    del x84
    x85 = np.zeros((nocc, nocc, nocc, nvir), dtype=types[float])
    ...
    del x89
    x90 = np.zeros((nocc, nocc, nvir, nvir, nvir), dtype=types[float])
    ...
    del x91
    x92 = np.zeros((nocc, nocc, nvir, nvir, nvir), dtype=types[float])
    ...
    del x95
    x96 = np.zeros((nocc, nocc, nvir, nvir, nvir), dtype=types[float])
    x96 += einsum(v.ovov, (0, 1, 2, 3), v.ovvv, (2, 4, 5, 1), denom3, (2, 6, 0, 3, 4, 1), (0, 6, 3, 4, 5))
    x97 = np.zeros((nocc, nvir), dtype=types[float])
    x97 += einsum(t2, (0, 1, 2, 3), x96, (1, 0, 3, 4, 2), (0, 4))
    e_pert += einsum(l1, (0, 1), x97, (1, 0), ()) * 2.0
    del x97
    x98 = np.zeros((nocc, nocc, nocc, nvir, nvir), dtype=types[float])
    ...
    del x103
    x104 = np.zeros((nocc, nocc, nvir, nvir, nvir), dtype=types[float])
    x104 += einsum(v.ovov, (0, 1, 2, 3), v.ovvv, (2, 3, 4, 5), denom3, (2, 6, 0, 3, 4, 1), (0, 6, 4, 1, 5))
    x105 = np.zeros((nocc, nvir), dtype=types[float])
    ...
    del x108
    x109 = np.zeros((nocc, nvir), dtype=types[float])
    ...
    del x115
    x116 = np.zeros((nocc, nvir), dtype=types[float])
    ...
    x118 = np.zeros((nocc, nvir), dtype=types[float])
    ...
    del x136
    x137 = np.zeros((nocc, nvir), dtype=types[float])
    x137 += einsum(t2, (0, 1, 2, 3), x127, (4, 0, 1, 3, 2), (4, 2))
    del x127
    e_pert += einsum(l1, (0, 1), x137, (1, 0), ()) * -2.0
    del x137
    x138 = np.zeros((nocc, nvir), dtype=types[float])
    ...
    del x139
    x140 = np.zeros((nocc, nvir), dtype=types[float])
    ...
    del x143
    x144 = np.zeros((nocc, nocc, nvir, nvir, nvir, nvir), dtype=types[float])
    ...
    x147 = np.zeros((nocc, nvir, nvir, nvir, nvir, nvir), dtype=types[float])
    x147 += einsum(v.ovvv, (0, 1, 2, 3), v.ovvv, (0, 4, 1, 5), (0, 1, 5, 2, 3, 4))
    x147 += einsum(v.ovvv, (0, 1, 2, 3), x146, (0, 1, 4, 5), (0, 1, 2, 5, 4, 3)) * -1.0
    del x146
    x148 = np.zeros((nocc, nocc, nvir, nvir, nvir, nvir), dtype=types[float])
    ...
    x153 = np.zeros((nocc, nocc, nvir, nvir, nvir, nvir), dtype=types[float])
    x153 += einsum(v.ovvv, (0, 1, 2, 3), x151, (4, 0, 1, 5), (0, 4, 1, 5, 2, 3))
    x153 += einsum(v.ovvv, (0, 1, 2, 3), x152, (4, 0, 2, 5), (0, 4, 2, 5, 3, 1)) * 0.5
    del x152
    x154 = np.zeros((nocc, nocc, nvir, nvir, nvir, nvir), dtype=types[float])
    ...
    del x155
    x156 = np.zeros((nocc, nocc, nvir, nvir, nvir, nvir), dtype=types[float])
    x156 += einsum(denom3, (0, 1, 2, 3, 4, 5), x153, (0, 1, 3, 6, 7, 4), (1, 2, 4, 5, 6, 7))
    del x153
    x157 = np.zeros((nocc, nvir, nvir, nvir), dtype=types[float])
    x157 += einsum(v.ovvv, (0, 1, 2, 3), (0, 1, 2, 3)) * 2.0
    x157 += einsum(v.ovvv, (0, 1, 2, 3), (0, 2, 1, 3)) * -1.0
    x158 = np.zeros((nocc, nocc, nvir, nvir, nvir, nvir), dtype=types[float])
    ...
    x162 = np.zeros((nocc, nvir, nvir, nvir), dtype=types[float])
    ...
    del x160, x164
    x165 = np.zeros((nocc, nocc, nvir, nvir), dtype=types[float])
    ...
    x167 = np.zeros((nocc, nocc, nvir, nvir, nvir), dtype=types[float])
