...
def update_amps(f=None, v=None, nocc=None, nvir=None, t1=None, t2=None, **kwargs):
    ...
    x30 += einsum("ij,kjab->kiab", x5, t2)
    del x5
    x31 = np.zeros((nocc, nocc, nvir, nvir), dtype=np.float64)
    x31 += einsum("ijab->ijba", x30) * -0.5
    del x30
    ...
    x32 -= einsum("ab,ijcb->ijca", x8, t2)
    del x8
    x39 = np.zeros((nocc, nocc, nvir, nvir), dtype=np.float64)
    x39 += einsum("ijab->ijab", x32)
    del x32
    ...
    x29 += einsum("ijab,kijl->klab", t2, x22)
    del x22
    x31 += einsum("ijab->ijba", x29) * -0.5
    del x29
    t2new += einsum("ijab->ijab", x31) * -1
    t2new += einsum("ijab->jiab", x31)
    del x31
...
def update_lams(f=None, v=None, nocc=None, nvir=None, t1=None, t2=None, l1=None, l2=None, **kwargs):
    ...
    x17 = np.zeros((nocc, nocc, nocc, nvir), dtype=np.float64)
    x17 += einsum("ia,ijkl->jlka", t1, x16) * -0.25
    del x16
    ...
    del x2
    x17 += einsum("ijab,kjlb->kila", t2, x1) * -1
    x27 = np.zeros((nocc, nvir), dtype=np.float64)
    x27 += einsum("ijab,ijka->kb", t2, x1)
    x44 = np.zeros((nocc, nocc, nvir, nvir), dtype=np.float64)
    x44 -= einsum("ijka,kljb->liba", v.ooov, x1)
    x49 = np.zeros((nocc, nocc, nvir, nvir), dtype=np.float64)
    x49 += einsum("ijab->ijab", x44)
    del x44
    x50 = np.zeros((nocc, nocc, nvir, nvir), dtype=np.float64)
    x50 -= einsum("ia,jkib->kjab", f.ov, x1)
    x54 = np.zeros((nocc, nocc, nvir, nvir), dtype=np.float64)
    x54 += einsum("ijab->ijab", x50)
    del x50
    l1new -= einsum("iajb,kjia->bk", v.ovov, x1)
    ...
    x30 += einsum("ij->ij", x3) * 2
    x36 = np.zeros((nocc, nocc, nvir, nvir), dtype=np.float64)
    x36 += einsum("ij,kjab->ikab", x3, v.oovv)
    x43 = np.zeros((nocc, nocc, nvir, nvir), dtype=np.float64)
    x43 -= einsum("ijab->ijba", x36)
    del x36
    x4 = np.zeros((nocc, nvir), dtype=np.float64)
    x4 += einsum("ia,jiba->jb", t1, v.oovv)
    x12 = np.zeros((nocc, nvir), dtype=np.float64)
    x12 += einsum("ia->ia", x4)
    x40 = np.zeros((nocc, nocc), dtype=np.float64)
    x40 += einsum("ia,ja->ij", t1, x4)
    x41 = np.zeros((nocc, nocc), dtype=np.float64)
    x41 += einsum("ij->ji", x40)
    del x40
    x49 += einsum("ai,jb->ijab", l1, x4)
    x53 = np.zeros((nocc, nocc, nvir, nvir), dtype=np.float64)
    x53 -= einsum("ia,jkib->kjba", x4, x1)
    x54 -= einsum("ijab->ijab", x53)
    del x53
    l1new -= einsum("ij,ja->ai", x3, x4)
    del x3
    del x4
    x5 = np.zeros((nocc, nocc, nvir, nvir), dtype=np.float64)
    x5 -= einsum("abij,kicb->jkac", l2, t2)
    l1new -= einsum("iabc,jiac->bj", v.ovvv, x5)
    del x5
    x6 = np.zeros((nocc, nocc, nvir, nvir), dtype=np.float64)
    x6 += einsum("ia,jbca->ijbc", t1, v.ovvv)
    x13 = np.zeros((nocc, nocc, nvir, nvir), dtype=np.float64)
    x13 += einsum("ijab->jiab", x6) * -0.5
    x47 = np.zeros((nocc, nocc, nvir, nvir), dtype=np.float64)
    x47 += einsum("ijab->ijab", x6)
    l1new += einsum("ijka,jkab->bi", x1, x6)
    del x6
    ...
    x10 += einsum("ijka->kjia", x7) * 0.5000000000000003
    x45 = np.zeros((nocc, nocc, nvir, nvir), dtype=np.float64)
    x45 += einsum("ijka,jlkb->ilab", x1, x7)
    del x1
    x49 -= einsum("ijab->ijab", x45)
    del x45
    x51 = np.zeros((nocc, nocc, nvir, nvir), dtype=np.float64)
    x51 -= einsum("ai,ijkb->kjab", l1, x7)
    x54 += einsum("ijab->ijab", x51)
    del x51
    x62 = np.zeros((nocc, nocc, nocc, nvir), dtype=np.float64)
    x62 += einsum("ijka->kjia", x7) * 0.5
    del x7
    x8 += einsum("ijka->kjia", v.ooov)
    x14 = np.zeros((nocc, nocc, nocc, nvir), dtype=np.float64)
    x14 += einsum("ijab,kila->ljkb", t2, x8)
    x25 = np.zeros((nocc, nvir), dtype=np.float64)
    x25 += einsum("ijab,kija->kb", t2, x8) * 0.5
    del x8
    x9 = np.zeros((nocc, nocc, nocc, nocc), dtype=np.float64)
    x9 += einsum("ijab,klab->ijkl", t2, v.oovv)
    x11 = np.zeros((nocc, nocc, nocc, nocc), dtype=np.float64)
    x11 += einsum("ijkl->lkji", x9) * -1
    x63 = np.zeros((nocc, nocc, nocc, nocc), dtype=np.float64)
    x63 += einsum("ijkl->lkji", x9) * 0.25
    del x9
    x10 += einsum("ijka->jika", v.ooov) * -1
    x11 += einsum("ia,jkla->kjil", t1, x10) * -4
    del x10
    x11 += einsum("ijkl->jilk", v.oooo) * -2
    x14 += einsum("ia,ijkl->jlka", t1, x11) * -0.25
    del x11
    x12 += einsum("ia->ia", f.ov)
    x14 += einsum("ia,jkab->ikjb", x12, t2) * 0.5
    x21 = np.zeros((nocc, nocc), dtype=np.float64)
    x21 += einsum("ia,ja->ji", t1, x12)
    x22 = np.zeros((nocc, nocc), dtype=np.float64)
    x22 += einsum("ij->ij", x21)
    del x21
    x25 += einsum("ia,ijab->jb", x12, t2) * -1
    del x12
    x13 += einsum("iajb->ijab", v.ovov)
    x14 += einsum("ia,jkba->jkib", t1, x13) * -1
    del x13
    x14 += einsum("iajk->ikja", v.ovoo) * 0.5
    x14 += einsum("ijab,kcab->kjic", t2, v.ovvv) * 0.25
    l1new += einsum("abij,kija->bk", l2, x14) * -1
    del x14
    x15 = np.zeros((nocc, nocc), dtype=np.float64)
    x15 += einsum("abij,ikab->jk", l2, t2)
    x17 += einsum("ia,jk->jika", t1, x15) * 0.5
    x26 += einsum("ij->ij", x15) * 0.5
    x27 += einsum("ia,ij->ja", t1, x26) * 2
    l1new += einsum("ia,ji->aj", f.ov, x26) * -1
    del x26
    x30 += einsum("ij->ij", x15)
    l1new += einsum("ij,jkia->ak", x30, v.ooov) * -0.5
    del x30
    x56 = np.zeros((nocc, nocc, nvir, nvir), dtype=np.float64)
    x56 -= einsum("ij,kjab->ikab", x15, v.oovv)
    del x15
    x57 = np.zeros((nocc, nocc, nvir, nvir), dtype=np.float64)
    x57 += einsum("ijab->ijba", x56) * -0.5
    del x56
    x17 += einsum("ai,jkba->ikjb", l1, t2) * -0.5
    l1new += einsum("ijab,kija->bk", v.oovv, x17) * -1
    del x17
    ...
    x19 -= einsum("ia,ijka->jk", t1, v.ooov)
    x22 += einsum("ij->ij", x19)
    x41 += einsum("ij->ij", x19)
    del x19
    x42 = np.zeros((nocc, nocc, nvir, nvir), dtype=np.float64)
    x42 += einsum("ij,abjk->ikab", x41, l2)
    del x41
    x43 -= einsum("ijab->jiba", x42)
    del x42
    x20 = np.zeros((nocc, nocc), dtype=np.float64)
    x20 -= einsum("ijab,kiab->jk", t2, v.oovv)
    x22 += einsum("ij->ji", x20) * 0.5
    x55 = np.zeros((nocc, nocc, nvir, nvir), dtype=np.float64)
    x55 += einsum("ij,abki->kjab", x20, l2)
    del x20
    x57 += einsum("ijab->ijba", x55) * -0.5
    del x55
    l2new += einsum("ijab->abij", x57) * -1
    l2new += einsum("ijab->abji", x57)
    del x57
    x22 += einsum("ij->ij", f.oo)
    x25 += einsum("ia,ij->ja", t1, x22)
    l1new += einsum("ai,ji->aj", l1, x22) * -1
    del x22
    x23 = np.zeros((nvir, nvir), dtype=np.float64)
    x23 += einsum("ia,ibca->bc", t1, v.ovvv)
    x24 = np.zeros((nvir, nvir), dtype=np.float64)
    x24 += einsum("ab->ab", x23) * -1
    x31 = np.zeros((nvir, nvir), dtype=np.float64)
    x31 -= einsum("ab->ab", x23)
    x52 = np.zeros((nocc, nocc, nvir, nvir), dtype=np.float64)
    x52 -= einsum("ab,caij->jicb", x23, l2)
    del x23
    x54 -= einsum("ijab->ijab", x52)
    del x52
    l2new += einsum("ijab->abij", x54)
    l2new -= einsum("ijab->baij", x54)
    del x54
    x24 += einsum("ab->ab", f.vv)
    x25 += einsum("ia,ba->ib", t1, x24) * -1
    del x24
    x25 += einsum("ai->ia", f.vo) * -1
    x25 += einsum("ia,ibja->jb", t1, v.ovov)
    x25 += einsum("ijab,icab->jc", t2, v.ovvv) * -0.5
    l1new += einsum("ia,abij->bj", x25, l2) * -1
    del x25
    x27 += einsum("ia->ia", t1) * -2
    x27 += einsum("ai,jiba->jb", l1, t2) * -2
    l1new += einsum("ia,ijab->bj", x27, v.oovv) * -0.5
    del x27
    x28 = np.zeros((nvir, nvir), dtype=np.float64)
    x28 -= einsum("abij,ijbc->ac", l2, t2)
    x29 = np.zeros((nvir, nvir), dtype=np.float64)
    x29 += einsum("ab->ab", x28) * 0.5
    x60 = np.zeros((nocc, nocc, nvir, nvir), dtype=np.float64)
    x60 += einsum("ab,ijcb->jiac", x28, v.oovv)
    del x28
    x61 = np.zeros((nocc, nocc, nvir, nvir), dtype=np.float64)
    x61 += einsum("ijab->ijab", x60) * 0.5
    del x60
    x29 += einsum("ai,ib->ab", l1, t1)
    l1new += einsum("ab,iabc->ci", x29, v.ovvv) * -1
    del x29
    x31 += einsum("ab->ab", f.vv)
    l1new += einsum("ai,ab->bi", l1, x31)
    del x31
    x32 = np.zeros((nocc, nocc, nvir, nvir), dtype=np.float64)
    x32 += einsum("ai,jkib->jkab", l1, v.ooov)
    x34 = np.zeros((nocc, nocc, nvir, nvir), dtype=np.float64)
    x34 -= einsum("ijab->jiab", x32)
    del x32
    x33 = np.zeros((nocc, nocc, nvir, nvir), dtype=np.float64)
    x33 += einsum("ab,caij->ijbc", f.vv, l2)
    x34 -= einsum("ijab->jiab", x33)
    del x33
    l2new -= einsum("ijab->abij", x34)
    l2new += einsum("ijab->baij", x34)
    del x34
    x35 = np.zeros((nocc, nocc, nvir, nvir), dtype=np.float64)
    x35 += einsum("ai,jabc->ijbc", l1, v.ovvv)
    x43 += einsum("ijab->ijba", x35)
    del x35
    x37 = np.zeros((nocc, nocc), dtype=np.float64)
    x37 += einsum("ia,ja->ij", f.ov, t1)
    x38 = np.zeros((nocc, nocc), dtype=np.float64)
    x38 += einsum("ij->ij", x37)
    del x37
    x38 += einsum("ij->ij", f.oo)
    x39 = np.zeros((nocc, nocc, nvir, nvir), dtype=np.float64)
    x39 += einsum("ij,abjk->ikab", x38, l2)
    del x38
    x43 += einsum("ijab->ijba", x39)
    del x39
    l2new += einsum("ijab->abij", x43)
    l2new -= einsum("ijab->abji", x43)
    del x43
    x46 = np.zeros((nocc, nocc, nvir, nvir), dtype=np.float64)
    x46 += einsum("ijab,kica->jkbc", t2, v.oovv)
    x47 += einsum("ijab->ijab", x46)
    del x46
    x47 -= einsum("iajb->jiab", v.ovov)
    x48 = np.zeros((nocc, nocc, nvir, nvir), dtype=np.float64)
    x48 += einsum("abij,ikac->kjcb", l2, x47)
    del x47
    x49 += einsum("ijab->jiba", x48)
    del x48
    x49 += einsum("ia,bj->ijab", f.ov, l1)
    l2new += einsum("ijab->abij", x49)
    l2new -= einsum("ijab->baij", x49)
    l2new -= einsum("ijab->abji", x49)
    l2new += einsum("ijab->baji", x49)
    del x49
    x58 = np.zeros((nvir, nvir), dtype=np.float64)
    x58 -= einsum("ijab,ijca->bc", t2, v.oovv)
    x59 = np.zeros((nocc, nocc, nvir, nvir), dtype=np.float64)
    x59 -= einsum("ab,caij->jicb", x58, l2)
    del x58
    x61 += einsum("ijab->ijab", x59) * 0.5
    del x59
    l2new += einsum("ijab->abij", x61) * -1
    l2new += einsum("ijab->baij", x61)
    del x61
    x62 += einsum("ijka->jika", v.ooov) * -1
    x63 += einsum("ia,jkla->kjli", t1, x62) * -1
    del x62
    x63 += einsum("ijkl->jilk", v.oooo) * 0.5
    l2new += einsum("abij,klij->balk", l2, x63)
    del x63
...
def make_rdm2_f(f=None, v=None, nocc=None, nvir=None, t1=None, t2=None, l1=None, l2=None, **kwargs):
    ...
    x16 += einsum("ia,ijkb->jkab", t1, x12)
    del x12
    ...
    x32 += einsum("ijab,ijkc->kcab", t2, x1)
    rdm2_f_ovvv += einsum("iabc->iacb", x32) * -0.5
    rdm2_f_vovv += einsum("iabc->aicb", x32) * 0.5
    del x32
    ...
    x7 += einsum("ia,ij->ja", t1, x3)
    x8 += einsum("ia->ia", x7)
    del x7
    rdm2_f_ooov += einsum("ij,ka->jkia", delta_oo, x8) * -0.5
    rdm2_f_ooov += einsum("ij,ka->kija", delta_oo, x8) * 0.5
    rdm2_f_oovo += einsum("ij,ka->jkai", delta_oo, x8) * 0.5
    rdm2_f_oovo += einsum("ij,ka->kiaj", delta_oo, x8) * -0.5
    rdm2_f_oovv += einsum("ia,jb->ijab", t1, x8) * -0.5000000000000003
    rdm2_f_oovv += einsum("ia,jb->ijba", t1, x8) * 0.5000000000000003
    rdm2_f_oovv += einsum("ia,jb->jiab", t1, x8) * 0.5000000000000003
    rdm2_f_oovv += einsum("ia,jb->jiba", t1, x8) * -0.5000000000000003
    del x8
    ...
    x9 += einsum("ai,jiba->jb", l1, t2)
    x11 += einsum("ia->ia", x9)
    del x9
    x17 -= einsum("ia,jb->ijab", t1, x11)
    ...
    del x17
    rdm2_f_ooov += einsum("ij,ka->jkia", delta_oo, x11)
    rdm2_f_ooov -= einsum("ij,ka->kija", delta_oo, x11)
    rdm2_f_oovo -= einsum("ij,ka->jkai", delta_oo, x11)
    rdm2_f_oovo += einsum("ij,ka->kiaj", delta_oo, x11)
    del x11
    x13 += einsum("ij,ka->jika", delta_oo, t1)
    rdm2_f_ooov += einsum("ijka->ikja", x13)
    rdm2_f_ooov -= einsum("ijka->kija", x13)
    rdm2_f_oovo -= einsum("ijka->ikaj", x13)
    rdm2_f_oovo += einsum("ijka->kiaj", x13)
    del x13
    x19 = np.zeros((nocc, nocc, nvir, nvir), dtype=np.float64)
    ...
    x31 += einsum("ab->ab", x30)
    rdm2_f_ovov += einsum("ij,ab->jaib", delta_oo, x31)
    rdm2_f_ovvo += einsum("ij,ab->jabi", delta_oo, x31) * -1
    rdm2_f_voov += einsum("ij,ab->ajib", delta_oo, x31) * -1
    rdm2_f_vovo += einsum("ij,ab->ajbi", delta_oo, x31)
    del x31
...
def hbar_ip(f=None, v=None, nocc=None, nvir=None, t1=None, t2=None, l1=None, l2=None, **kwargs):
    ...
    x0 = np.zeros((nocc, nocc), dtype=np.float64)
    x0 += einsum("ijab,kjab->ik", t2, v.oovv)
    x10 = np.zeros((nocc, nvir), dtype=np.float64)
    x10 += einsum("ia,ji->ja", t1, x0)
    x11 = np.zeros((nocc, nvir), dtype=np.float64)
    x11 += einsum("ia->ia", x10)
    del x10
    x38 = np.zeros((nocc, nocc), dtype=np.float64)
    ...
    x39 += einsum("ij,kl->kijl", delta_oo, x1)
    x39 += einsum("ij,kl->kilj", delta_oo, x1) * -1
    x39 += einsum("ij,kl->jkil", delta_oo, x1) * -1
    x39 += einsum("ij,kl->jkli", delta_oo, x1)
    h11 -= einsum("ij->ji", x1)
    del x1
    x2 = np.zeros((nocc, nvir), dtype=np.float64)
    x2 -= einsum("ia,jiab->jb", t1, v.oovv)
    x3 = np.zeros((nocc, nvir), dtype=np.float64)
    x3 += einsum("ia->ia", x2)
    x18 = np.zeros((nocc, nocc), dtype=np.float64)
    x18 += einsum("ia,ja->ij", t1, x2)
    x19 += einsum("ij->ji", x18)
    x20 = np.zeros((nocc, nvir), dtype=np.float64)
    x20 += einsum("ia,ij->ja", t1, x19)
    del x19
    x21 = np.zeros((nocc, nvir), dtype=np.float64)
    x21 += einsum("ia->ia", x20)
    del x20
    x38 += einsum("ij->ij", x18) * 2
    del x18
    x39 += einsum("ij,kl->lijk", delta_oo, x38) * 0.5
    x39 += einsum("ij,kl->likj", delta_oo, x38) * -0.5
    x39 += einsum("ij,kl->jlik", delta_oo, x38) * -0.5
    x39 += einsum("ij,kl->jlki", delta_oo, x38) * 0.5
    del x38
    x40 = np.zeros((nocc, nvir), dtype=np.float64)
    x40 += einsum("ia->ia", x2)
    h12 = np.zeros((nocc, nocc, nocc, nvir), dtype=np.float64)
    h12 -= einsum("ij,ka->jkia", delta_oo, x2)
    h12 += einsum("ij,ka->jika", delta_oo, x2)
    del x2
    x3 += einsum("ia->ia", f.ov)
    h11 -= einsum("ia,ja->ij", t1, x3)
    h21 = np.zeros((nocc, nocc, nvir, nocc), dtype=np.float64)
    h21 += einsum("ia,jkab->kjbi", x3, t2)
    del x3
    x4 = np.zeros((nocc, nocc, nocc, nvir), dtype=np.float64)
    x4 += einsum("ia,jkba->ijkb", t1, v.oovv)
    x29 = np.zeros((nocc, nocc, nocc, nocc), dtype=np.float64)
    x29 += einsum("ia,jkla->jilk", t1, x4)
    x30 = np.zeros((nocc, nocc, nocc, nocc), dtype=np.float64)
    x30 += einsum("ijkl->klji", x29) * -2
    x39 += einsum("ijkl->klji", x29) * -1
    del x29
    x43 = np.zeros((nocc, nocc, nocc, nvir), dtype=np.float64)
    x43 -= einsum("ijka->ikja", x4)
    h12 -= einsum("ijka->ikja", x4)
    del x4
    x5 = np.zeros((nocc, nocc, nvir, nvir), dtype=np.float64)
    x5 -= einsum("ia,jbac->ijbc", t1, v.ovvv)
    x45 = np.zeros((nocc, nocc, nvir, nvir), dtype=np.float64)
    x45 -= einsum("ijab->jiab", x5)
    x46 = np.zeros((nocc, nocc, nvir, nvir), dtype=np.float64)
    x46 += einsum("ijab->jiab", x5)
    h21 -= einsum("ia,jkba->ijbk", t1, x5)
    del x5
    x6 = np.zeros((nocc, nvir), dtype=np.float64)
    x6 += einsum("ijab,ijkb->ka", t2, v.ooov)
    x11 += einsum("ia->ia", x6)
    del x6
    x7 = np.zeros((nocc, nvir), dtype=np.float64)
    x7 += einsum("ijab,jcab->ic", t2, v.ovvv)
    x11 += einsum("ia->ia", x7)
    del x7
    x8 = np.zeros((nvir, nvir), dtype=np.float64)
    x8 -= einsum("ijab,ijbc->ac", t2, v.oovv)
    x9 = np.zeros((nocc, nvir), dtype=np.float64)
    x9 += einsum("ia,ba->ib", t1, x8)
    x11 += einsum("ia->ia", x9)
    del x9
    h21 += einsum("ij,ka->kiaj", delta_oo, x11) * 0.5
    h21 += einsum("ij,ka->jkai", delta_oo, x11) * -0.5
    del x11
    x41 = np.zeros((nvir, nvir), dtype=np.float64)
    x41 += einsum("ab->ab", x8) * 0.5
    del x8
    x12 = np.zeros((nocc, nvir), dtype=np.float64)
    x12 += einsum("ia,jiba->jb", f.ov, t2)
    x21 -= einsum("ia->ia", x12)
    del x12
    x13 = np.zeros((nvir, nvir), dtype=np.float64)
    x13 -= einsum("ia,ibac->bc", t1, v.ovvv)
    x14 = np.zeros((nocc, nvir), dtype=np.float64)
    x14 += einsum("ia,ba->ib", t1, x13)
    x21 += einsum("ia->ia", x14)
    del x14
    x41 += einsum("ab->ab", x13)
    del x13
    x15 = np.zeros((nocc, nocc, nvir, nvir), dtype=np.float64)
    x15 -= einsum("ijab,kjbc->ikac", t2, v.oovv)
    x16 = np.zeros((nocc, nocc, nvir, nvir), dtype=np.float64)
    x16 -= einsum("ijab->jiab", x15)
    x22 = np.zeros((nocc, nocc, nocc, nvir), dtype=np.float64)
    x22 += einsum("ia,jkba->ijkb", t1, x15)
    h21 -= einsum("ijka->ijak", x22)
    h21 += einsum("ijka->jiak", x22)
    del x22
    h22 = np.zeros((nocc, nocc, nvir, nocc, nocc, nvir), dtype=np.float64)
    h22 -= einsum("ij,klab->kiajlb", delta_oo, x15)
    h22 += einsum("ij,klab->kialjb", delta_oo, x15)
    h22 += einsum("ij,klab->jkailb", delta_oo, x15)
    h22 -= einsum("ij,klab->jkalib", delta_oo, x15)
    del x15
    x16 += einsum("iajb->ijab", v.ovov)
    x17 = np.zeros((nocc, nvir), dtype=np.float64)
    x17 += einsum("ia,ijba->jb", t1, x16)
    del x16
    x21 += einsum("ia->ia", x17)
    del x17
    h21 += einsum("ij,ka->kiaj", delta_oo, x21)
    h21 -= einsum("ij,ka->jkai", delta_oo, x21)
    del x21
    x23 = np.zeros((nocc, nocc, nocc, nvir), dtype=np.float64)
    x23 += einsum("ia,jbka->ijkb", t1, v.ovov)
    x27 = np.zeros((nocc, nocc, nocc, nvir), dtype=np.float64)
    x27 += einsum("ijka->ijka", x23)
    del x23
    x24 = np.zeros((nocc, nocc, nocc, nvir), dtype=np.float64)
    x24 += einsum("ijab,kjlb->ikla", t2, v.ooov)
    x27 += einsum("ijka->ijka", x24)
    del x24
    x25 = np.zeros((nocc, nocc, nocc, nocc), dtype=np.float64)
    x25 += einsum("ia,jkla->ijkl", t1, v.ooov)
    x26 = np.zeros((nocc, nocc, nocc, nvir), dtype=np.float64)
    x26 += einsum("ia,jkil->jkla", t1, x25)
    x27 -= einsum("ijka->ijka", x26)
    del x26
    h21 += einsum("ijka->ikaj", x27)
    h21 -= einsum("ijka->kiaj", x27)
    del x27
    x39 += einsum("ijkl->kjil", x25)
    x39 += einsum("ijkl->kjli", x25) * -1
    del x25
    x28 = np.zeros((nocc, nocc, nocc, nocc), dtype=np.float64)
    x28 += einsum("ijab,klab->ijkl", t2, v.oovv)
    x30 += einsum("ijkl->lkji", x28)
    x39 += einsum("ijkl->lkji", x28) * 0.5
    del x28
    x30 += einsum("ijkl->jilk", v.oooo) * 2
    h21 += einsum("ia,ijkl->lkaj", t1, x30) * 0.5
    del x30
    x31 = np.zeros((nocc, nvir), dtype=np.float64)
    x31 += einsum("ab,ib->ia", f.vv, t1)
    x35 = np.zeros((nocc, nvir), dtype=np.float64)
    x35 -= einsum("ia->ia", x31)
    x36 = np.zeros((nocc, nvir), dtype=np.float64)
    x36 += einsum("ia->ia", x31)
    del x31
    x32 = np.zeros((nocc, nocc), dtype=np.float64)
    x32 += einsum("ia,ja->ij", f.ov, t1)
    x33 = np.zeros((nocc, nocc), dtype=np.float64)
    x33 += einsum("ij->ij", x32)
    x37 = np.zeros((nocc, nocc), dtype=np.float64)
    x37 += einsum("ij->ij", x32)
    del x32
    x33 += einsum("ij->ij", f.oo)
    x34 = np.zeros((nocc, nvir), dtype=np.float64)
    x34 += einsum("ia,ij->ja", t1, x33)
    del x33
    x35 += einsum("ia->ia", x34)
    x36 -= einsum("ia->ia", x34)
    del x34
    x35 -= einsum("ai->ia", f.vo)
    h21 -= einsum("ij,ka->jkai", delta_oo, x35)
    del x35
    x36 += einsum("ai->ia", f.vo)
    h21 -= einsum("ij,ka->kjai", delta_oo, x36)
    del x36
    x37 += einsum("ij->ij", f.oo)
    x39 += einsum("ij,kl->jkil", delta_oo, x37) * -1
    x39 += einsum("ij,kl->kjil", delta_oo, x37)
    x39 += einsum("ij,kl->jkli", delta_oo, x37)
    x39 += einsum("ij,kl->kjli", delta_oo, x37) * -1
    del x37
    x39 += einsum("ijkl->jilk", v.oooo)
    h22 += einsum("ab,ijkl->lkbjia", delta_vv, x39)
    del x39
    x40 += einsum("ia->ia", f.ov)
    x41 += einsum("ia,ib->ab", t1, x40)
    del x40
    x41 += einsum("ab->ab", f.vv) * -1
    x42 = np.zeros((nocc, nocc, nocc, nocc), dtype=np.float64)
    x42 -= einsum("ij,kl->jkli", delta_oo, delta_oo)
    x42 += einsum("ij,kl->lkji", delta_oo, delta_oo)
    h22 += einsum("ab,ijkl->jlakib", x41, x42)
    del x41
    del x42
    x43 += einsum("ijka->kjia", v.ooov)
    x44 = np.zeros((nocc, nocc, nvir, nvir), dtype=np.float64)
    x44 += einsum("ia,jikb->jkba", t1, x43)
    del x43
    x45 -= einsum("ijab->jiba", x44)
    x46 += einsum("ijab->jiba", x44)
    del x44
    x45 += einsum("iajb->ijab", v.ovov)
    h22 += einsum("ij,klab->ljaikb", delta_oo, x45)
    h22 -= einsum("ij,klab->jlaikb", delta_oo, x45)
    del x45
    x46 -= einsum("iajb->ijab", v.ovov)
    h22 -= einsum("ij,klab->jlakib", delta_oo, x46)
    h22 += einsum("ij,klab->ljakib", delta_oo, x46)
    del x46
    ...
    h21 += einsum("ijab,kcab->jick", t2, v.ovvv) * 0.5
    h22 -= einsum("ijab,klcb->jialkc", t2, v.oovv)
...
def hbar_ea(f=None, v=None, nocc=None, nvir=None, t1=None, t2=None, l1=None, l2=None, **kwargs):
    ...
    x1 += einsum("ijab,ijcb->ac", t2, v.oovv)
    x7 = np.zeros((nocc, nvir), dtype=np.float64)
    x7 += einsum("ia,ba->ib", t1, x1)
    x10 = np.zeros((nocc, nvir), dtype=np.float64)
    x10 += einsum("ia->ia", x7)
    del x7
    x39 += einsum("ab->ab", x1) * 0.5
    h11 += einsum("ab->ab", x1) * -0.5
    del x1
    x2 = np.zeros((nocc, nvir), dtype=np.float64)
    ...
    x29 += einsum("ia,ja->ij", t1, x3)
    x30 = np.zeros((nocc, nocc), dtype=np.float64)
    x30 += einsum("ij->ji", x29)
    del x29
    h11 -= einsum("ia,ib->ab", t1, x3)
    h21 = np.zeros((nvir, nvir, nocc, nvir), dtype=np.float64)
    h21 += einsum("ia,ijbc->cbja", x3, t2)
    del x3
    x4 = np.zeros((nocc, nvir, nvir, nvir), dtype=np.float64)
    x4 += einsum("ia,jibc->jabc", t1, v.oovv)
    x43 = np.zeros((nocc, nvir, nvir, nvir), dtype=np.float64)
    x43 += einsum("iabc->iacb", x4)
    h12 -= einsum("iabc->acbi", x4)
    del x4
    x5 = np.zeros((nocc, nvir), dtype=np.float64)
    x5 += einsum("ijab,ijkb->ka", t2, v.ooov)
    x10 += einsum("ia->ia", x5)
    del x5
    x6 = np.zeros((nocc, nvir), dtype=np.float64)
    x6 += einsum("ijab,jcab->ic", t2, v.ovvv)
    x10 += einsum("ia->ia", x6)
    del x6
    x8 = np.zeros((nocc, nocc), dtype=np.float64)
    x8 -= einsum("ijab,jkab->ik", t2, v.oovv)
    x9 = np.zeros((nocc, nvir), dtype=np.float64)
    x9 += einsum("ia,ji->ja", t1, x8)
    x10 += einsum("ia->ia", x9)
    del x9
    h21 += einsum("ab,ic->cbia", delta_vv, x10) * 0.5
    h21 += einsum("ab,ic->acib", delta_vv, x10) * -0.5
    del x10
    x37 = np.zeros((nocc, nocc), dtype=np.float64)
    x37 += einsum("ij->ji", x8) * 0.5
    del x8
    x12 = np.zeros((nocc, nocc, nvir, nvir), dtype=np.float64)
    x12 -= einsum("ijab,jkcb->ikac", t2, v.oovv)
    x13 = np.zeros((nocc, nvir), dtype=np.float64)
    x13 += einsum("ia,jiba->jb", t1, x12)
    x14 -= einsum("ia->ia", x13)
    del x13
    h21 += einsum("ab,ic->cbia", delta_vv, x14)
    h21 -= einsum("ab,ic->acib", delta_vv, x14)
    del x14
    x17 = np.zeros((nocc, nocc, nvir, nvir), dtype=np.float64)
    x17 += einsum("ijab->ijab", x12)
    x40 = np.zeros((nocc, nocc, nvir, nvir), dtype=np.float64)
    x40 += einsum("ijab->ijab", x12) * -1
    x41 = np.zeros((nocc, nocc, nvir, nvir), dtype=np.float64)
    x41 += einsum("ijab->jiab", x12) * -1
    x42 = np.zeros((nocc, nocc, nvir, nvir), dtype=np.float64)
    x42 += einsum("ijab->ijab", x12) * -1
    del x12
    x15 = np.zeros((nocc, nvir, nvir, nvir), dtype=np.float64)
    x15 += einsum("ijab,jcdb->iacd", t2, v.ovvv)
    x19 = np.zeros((nocc, nvir, nvir, nvir), dtype=np.float64)
    x19 -= einsum("iabc->iabc", x15)
    del x15
    x16 = np.zeros((nocc, nocc, nvir, nvir), dtype=np.float64)
    x16 += einsum("ia,jbca->ijbc", t1, v.ovvv)
    x17 += einsum("ijab->ijab", x16)
    x18 = np.zeros((nocc, nvir, nvir, nvir), dtype=np.float64)
    x18 += einsum("ia,jibc->jbca", t1, x17)
    del x17
    x19 += einsum("iabc->icab", x18)
    del x18
    h21 += einsum("iabc->baic", x19)
    h21 -= einsum("iabc->abic", x19)
    del x19
    x40 += einsum("ijab->ijab", x16) * -1
    x41 += einsum("ijab->jiab", x16) * -1
    x42 += einsum("ijab->ijab", x16) * -1
    del x16
    x20 = np.zeros((nvir, nvir, nvir, nvir), dtype=np.float64)
    x20 += einsum("ijab,ijcd->abcd", t2, v.oovv)
    x21 = np.zeros((nvir, nvir, nvir, nvir), dtype=np.float64)
    x21 += einsum("abcd->badc", x20) * 0.5
    x44 = np.zeros((nvir, nvir, nvir, nvir), dtype=np.float64)
    x44 += einsum("abcd->badc", x20) * 0.5
    del x20
    x21 += einsum("abcd->badc", v.vvvv)
    h21 += einsum("ia,bcad->cbid", t1, x21)
    del x21
    x22 = np.zeros((nocc, nocc, nocc, nvir), dtype=np.float64)
    x22 += einsum("ia,jkba->ijkb", t1, v.oovv)
    x23 = np.zeros((nocc, nocc, nocc, nvir), dtype=np.float64)
    x23 -= einsum("ijka->ikja", x22)
    x33 = np.zeros((nocc, nocc, nocc, nvir), dtype=np.float64)
    x33 += einsum("ijka->ikja", x22) * -1
    del x22
    x23 += einsum("ijka->kjia", v.ooov)
    x24 = np.zeros((nocc, nocc, nvir, nvir), dtype=np.float64)
    x24 -= einsum("ia,jikb->jkab", t1, x23)
    del x23
    x24 += einsum("iajb->jiab", v.ovov)
    h21 -= einsum("ia,jibc->bajc", t1, x24)
    del x24
    x25 = np.zeros((nocc, nvir), dtype=np.float64)
    x25 += einsum("ab,ib->ia", f.vv, t1)
    x32 = np.zeros((nocc, nvir), dtype=np.float64)
    x32 -= einsum("ia->ia", x25)
    del x25
    x26 = np.zeros((nocc, nvir), dtype=np.float64)
    x26 += einsum("ia,ibja->jb", t1, v.ovov)
    x32 += einsum("ia->ia", x26)
    del x26
    x27 = np.zeros((nocc, nvir), dtype=np.float64)
    x27 += einsum("ia,jiba->jb", f.ov, t2)
    x32 -= einsum("ia->ia", x27)
    del x27
    x28 = np.zeros((nocc, nocc), dtype=np.float64)
    x28 -= einsum("ia,ijka->jk", t1, v.ooov)
    x30 += einsum("ij->ij", x28)
    x37 += einsum("ij->ij", x28)
    del x28
    x30 += einsum("ij->ij", f.oo)
    x31 = np.zeros((nocc, nvir), dtype=np.float64)
    x31 += einsum("ia,ij->ja", t1, x30)
    del x30
    x32 += einsum("ia->ia", x31)
    del x31
    x32 -= einsum("ai->ia", f.vo)
    h21 += einsum("ab,ic->cbia", delta_vv, x32)
    h21 -= einsum("ab,ic->bcia", delta_vv, x32)
    del x32
    x33 += einsum("ijka->kjia", v.ooov)
    x34 = np.zeros((nocc, nocc, nvir, nvir), dtype=np.float64)
    x34 += einsum("ia,jikb->jkba", t1, x33)
    del x33
    x40 += einsum("ijab->ijba", x34) * -1
    x41 += einsum("ijab->jiba", x34) * -1
    x42 += einsum("ijab->ijba", x34) * -1
    del x34
    x35 += einsum("ia->ia", f.ov)
    x36 = np.zeros((nocc, nocc), dtype=np.float64)
    x36 += einsum("ia,ja->ji", t1, x35)
    x37 += einsum("ij->ij", x36)
    del x36
    x38 = np.zeros((nvir, nvir), dtype=np.float64)
    x38 += einsum("ia,ib->ba", t1, x35)
    del x35
    x39 += einsum("ab->ba", x38)
    del x38
    x37 += einsum("ij->ij", f.oo)
    x40 += einsum("ab,ij->jiba", delta_vv, x37)
    del x37
    x39 += einsum("ab->ab", f.vv) * -1
    x40 += einsum("ij,ab->jiab", delta_oo, x39)
    x41 += einsum("ij,ab->jiab", delta_oo, x39)
    x42 += einsum("ij,ab->jiab", delta_oo, x39)
    del x39
    x40 += einsum("iajb->jiab", v.ovov)
    h22 = np.zeros((nvir, nvir, nocc, nvir, nvir, nocc), dtype=np.float64)
    h22 += einsum("ab,ijcd->cbidaj", delta_vv, x40) * -1
    h22 += einsum("ab,ijcd->cbiadj", delta_vv, x40)
    del x40
    x41 += einsum("iajb->ijab", v.ovov)
    h22 += einsum("ab,ijcd->bcjadi", delta_vv, x41) * -1
    del x41
    x42 += einsum("iajb->jiab", v.ovov)
    h22 += einsum("ab,ijcd->bcidaj", delta_vv, x42)
    del x42
    x43 += einsum("iabc->iacb", v.ovvv) * -1
    x44 += einsum("ia,ibcd->abdc", t1, x43)
    del x43
    x44 += einsum("abcd->badc", v.vvvv)
    x44 += einsum("ia,ibcd->badc", t1, v.ovvv) * -1
    h22 += einsum("ij,abcd->bajdci", delta_oo, x44)
    del x44
    h11 += einsum("ab->ab", f.vv)
    h12 += einsum("iabc->acbi", v.ovvv)
    h12 += einsum("ab,ic->baci", delta_vv, f.ov)
    h12 -= einsum("ab,ic->bcai", delta_vv, f.ov)
    h21 += einsum("ijab,ijkc->bakc", t2, v.ooov) * 0.5
    h21 += einsum("ia,ibjc->abjc", t1, v.ovov)
