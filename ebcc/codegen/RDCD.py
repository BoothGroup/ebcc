"""
Code generated by `albert`:
https://github.com/obackhouse/albert

  * date: 2024-10-20T09:35:50.454783
  * python version: 3.10.15 (main, Sep  9 2024, 03:02:45) [GCC 11.4.0]
  * albert version: 0.0.0
  * caller: /opt/hostedtoolcache/Python/3.10.15/x64/lib/python3.10/site-packages/albert/codegen/einsum.py
  * node: fv-az985-968
  * system: Linux
  * processor: x86_64
  * release: 6.5.0-1025-azure
"""

from ebcc import numpy as np
from ebcc.util import pack_2e, einsum, dirsum, Namespace


def energy(t2=None, v=None, **kwargs):
    """
    Code generated by `albert` 0.0.0 on 2024-10-20T09:35:55.555123.

    Parameters
    ----------
    t2 : array
        T2 amplitudes.
    v : array
        Electron repulsion integrals.

    Returns
    -------
    e_cc : float
        Coupled cluster energy.
    """

    e_cc = einsum(v.ovov, (0, 1, 2, 3), t2, (0, 2, 1, 3), ()) * 2
    e_cc += einsum(v.ovov, (0, 1, 2, 3), t2, (0, 2, 3, 1), ()) * -1

    return e_cc

def update_amps(f=None, t2=None, v=None, **kwargs):
    """
    Code generated by `albert` 0.0.0 on 2024-10-20T09:36:57.557709.

    Parameters
    ----------
    f : array
        Fock matrix.
    t2 : array
        T2 amplitudes.
    v : array
        Electron repulsion integrals.

    Returns
    -------
    t2new : array
        Updated T2 residuals.
    """

    t2new = einsum(v.oovv, (0, 1, 2, 3), t2, (4, 1, 5, 3), (0, 4, 2, 5)) * -1
    tmp0 = einsum(t2, (0, 1, 2, 3), v.ovov, (4, 5, 1, 2), (0, 4, 3, 5))
    t2new += np.transpose(tmp0, (1, 0, 3, 2)) * -1
    tmp1 = einsum(t2, (0, 1, 2, 3), v.ovov, (4, 5, 1, 3), (0, 4, 2, 5))
    t2new += np.transpose(tmp1, (1, 0, 3, 2)) * 2
    t2new += np.transpose(v.ovov, (0, 2, 1, 3))
    t2new += einsum(v.vvvv, (0, 1, 2, 3), t2, (4, 5, 1, 3), (4, 5, 0, 2))
    t2new += einsum(t2, (0, 1, 2, 3), v.oooo, (4, 1, 5, 0), (4, 5, 3, 2))
    tmp2 = einsum(f.vv, (0, 1), t2, (2, 3, 4, 1), (2, 3, 0, 4))
    tmp4 = np.copy(tmp2)
    del tmp2
    tmp3 = einsum(t2, (0, 1, 2, 3), v.oovv, (4, 1, 5, 2), (0, 4, 3, 5))
    tmp4 += tmp3 * -1
    del tmp3
    t2new += np.transpose(tmp4, (0, 1, 3, 2))
    t2new += np.transpose(tmp4, (1, 0, 2, 3))
    del tmp4
    tmp5 = einsum(tmp0, (0, 1, 2, 3), t2, (4, 1, 5, 3), (4, 0, 5, 2))
    tmp11 = np.copy(tmp5) * 2
    del tmp5
    tmp6 = np.copy(np.transpose(v.ovov, (0, 2, 3, 1)))
    tmp6 += np.transpose(v.ovov, (0, 2, 1, 3)) * -0.5
    tmp7 = einsum(t2, (0, 1, 2, 3), tmp6, (0, 1, 3, 4), (2, 4))
    tmp8 = einsum(tmp7, (0, 1), t2, (2, 3, 1, 4), (2, 3, 4, 0))
    del tmp7
    tmp11 += np.transpose(tmp8, (1, 0, 2, 3))
    del tmp8
    tmp9 = einsum(tmp6, (0, 1, 2, 3), t2, (4, 0, 2, 3), (4, 1)) * 2
    del tmp6
    tmp10 = einsum(tmp9, (0, 1), t2, (2, 1, 3, 4), (2, 0, 4, 3)) * 0.5
    del tmp9
    tmp11 += np.transpose(tmp10, (0, 1, 3, 2))
    del tmp10
    t2new += tmp11 * -1
    t2new += np.transpose(tmp11, (1, 0, 3, 2)) * -1
    del tmp11
    tmp12 = einsum(t2, (0, 1, 2, 3), f.oo, (4, 1), (4, 0, 2, 3))
    t2new += np.transpose(tmp12, (0, 1, 3, 2)) * -1
    t2new += np.transpose(tmp12, (1, 0, 2, 3)) * -1
    del tmp12
    tmp13 = np.copy(np.transpose(v.ovov, (0, 2, 1, 3))) * -1
    tmp13 += tmp0
    del tmp0
    t2new += einsum(tmp13, (0, 1, 2, 3), t2, (4, 1, 3, 5), (4, 0, 5, 2))
    del tmp13
    tmp14 = np.copy(np.transpose(v.ovov, (0, 2, 1, 3))) * 2
    tmp14 += v.oovv * -1
    tmp14 += tmp1 * 4
    del tmp1
    t2new += einsum(t2, (0, 1, 2, 3), tmp14, (4, 1, 5, 3), (0, 4, 2, 5))
    del tmp14

    return {f"t2new": t2new}

