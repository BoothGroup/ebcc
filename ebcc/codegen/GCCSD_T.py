...
def energy_perturbative(f=None, v=None, nocc=None, nvir=None, t1=None, t2=None, l1=None, l2=None, **kwargs):
    ...
    t3 += np.einsum("ilab,jklc->ijkabc", t2, v.ooov) * -1.0
    t3 += np.einsum("ilac,jklb->ijkabc", t2, v.ooov)
    t3 += np.einsum("ilbc,jkla->ijkabc", t2, v.ooov) * -1.0
    t3 += np.einsum("jlab,iklc->ijkabc", t2, v.ooov)
    t3 += np.einsum("jlac,iklb->ijkabc", t2, v.ooov) * -1.0
    t3 += np.einsum("jlbc,ikla->ijkabc", t2, v.ooov)
    t3 += np.einsum("klab,ijlc->ijkabc", t2, v.ooov) * -1.0
    t3 += np.einsum("klac,ijlb->ijkabc", t2, v.ooov)
    t3 += np.einsum("klbc,ijla->ijkabc", t2, v.ooov) * -1.0
    t3 += np.einsum("ikad,jdbc->ijkabc", t2, v.ovvv)
    t3 += np.einsum("ikbd,jdac->ijkabc", t2, v.ovvv) * -1.0
    t3 += np.einsum("ikcd,jdab->ijkabc", t2, v.ovvv)
    t3 += np.einsum("ijad,kdbc->ijkabc", t2, v.ovvv) * -1.0
    t3 += np.einsum("ijbd,kdac->ijkabc", t2, v.ovvv)
    t3 += np.einsum("ijcd,kdab->ijkabc", t2, v.ovvv) * -1.0
    t3 += np.einsum("jkad,idbc->ijkabc", t2, v.ovvv) * -1.0
    t3 += np.einsum("jkbd,idac->ijkabc", t2, v.ovvv)
    t3 += np.einsum("jkcd,idab->ijkabc", t2, v.ovvv) * -1.0

    # energy
    e_pert = 0
    e_pert += np.einsum("ai,jkbc,ijkabc->", l1, v.oovv, t3) * 0.25
    e_pert += np.einsum("abij,kljc,iklabc->", l2, v.ooov, t3) * -0.25
    e_pert += np.einsum("abij,kbcd,ijkacd->", l2, v.ovvv, t3) * -0.25
