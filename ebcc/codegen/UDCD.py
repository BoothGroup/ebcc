...
def update_amps(f=None, v=None, nocc=None, nvir=None, t2=None, **kwargs):
    ...
    del x24
    x25 = np.zeros((nocc[0], nocc[1], nvir[0], nvir[1]), dtype=np.float64)
    ...
    del x25
    x26 = np.zeros((nvir[1], nvir[1]), dtype=np.float64)
    ...
    x49 = np.zeros((nocc[0], nocc[1], nvir[0], nvir[1]), dtype=np.float64)
