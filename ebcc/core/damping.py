...
class DIIS(diis.DIIS, Generic[T]):
    ...
    # TODO clean this up and fix the type hints
    ...
    def update(self, x: tuple[T, ...], xerr: Optional[tuple[T, ...]] = None) -> tuple[T, ...]:
        ...
        if self.damping:
            ...
            if nd > 1:
                ...
                xnew = tuple(
                    (1.0 - self.damping) * xnewi + self.damping * xprevi
                    for xnewi, xprevi in zip(xnew, xprev)
                )

        return xnew

    def extrapolate(self, nd: Optional[int] = None) -> tuple[T, ...]:
