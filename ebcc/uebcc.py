...
@util.inherit_docstrings
class SplitUEBCC(UEBCC, rebcc.SplitREBCC):

    @classmethod
    def from_rebbc(cls, ucc):
    ...
    def init_amps(self, eris=None):
        ...
        def get_e_ia(spin, key):
            ei = np.diag(getattr(getattr(self.fock, spin), key[0] * 2))
            ea = np.diag(getattr(getattr(self.fock, spin), key[1] * 2))
        ...
        for n, keys in self.ansatz.split_cluster_ranks(spin=self.spin_type)[0]:
            ...
            for spin in util.generate_spin_combinations(n, unique=True):
                setattr(amplitudes["t%d" % n], spin, util.Namespace())
            for key in keys:
                if n == 1:
                    tn = util.Namespace(
                        aa=getattr(self.fock.aa, key) / get_e_ia("aa", key),
                        bb=getattr(self.fock.bb, key) / get_e_ia("bb", key),
                    )
                    setattr(getattr(amplitudes["t%d" % n], "aa"), key, tn.aa)
                    setattr(getattr(amplitudes["t%d" % n], "bb"), key, tn.bb)
                elif n == 2:
                    key_t = key[0] + key[2] + key[1] + key[3]
                    e_ijab = util.Namespace(
                        aaaa=lib.direct_sum("ia,jb->ijab", get_e_ia("aa", key_t[:2]), get_e_ia("aa", key_t[2:])),
                        abab=lib.direct_sum("ia,jb->ijab", get_e_ia("aa", key_t[:2]), get_e_ia("bb", key_t[2:])),
                        bbbb=lib.direct_sum("ia,jb->ijab", get_e_ia("bb", key_t[:2]), get_e_ia("bb", key_t[2:])),
                    )
                    tn = util.Namespace(
                        aaaa=getattr(eris.aaaa, key_t).swapaxes(1, 2) / e_ijab.aaaa,
                        abab=getattr(eris.aabb, key_t).swapaxes(1, 2) / e_ijab.abab,
                    ...
                    if key[0] == key[1]:
                        ...
                        tn.aaaa = 0.5 * (tn.aaaa - tn.aaaa.swapaxes(2, 3))
                        tn.bbbb = 0.5 * (tn.bbbb - tn.bbbb.swapaxes(2, 3))
                    else:
                        pass
                    setattr(getattr(amplitudes["t%d" % n], "aaaa"), key, tn.aaaa)
                    setattr(getattr(amplitudes["t%d" % n], "abab"), key, tn.abab)
                    setattr(getattr(amplitudes["t%d" % n], "bbbb"), key, tn.bbbb)
                else:
                    for comb in util.generate_spin_combinations(n, unique=True):
                        occ = lambda space, k: space.naocc if k == "O" else space.niocc
                        vir = lambda space, k: space.navir if k == "O" else space.nivir
                        shape = tuple(occ(self.space["ab".index(s)], k) for s, k in zip(comb[:n], key[:n]))
                        shape += tuple(vir(self.space["ab".index(s)], k) for s, k in zip(comb[n:], key[n:]))
                        amp = np.zeros(shape)
                        setattr(getattr(amplitudes["t%d" % n], comb), key, amp)
    ...
    def amplitudes_to_vector(self, amplitudes):
        # FIXME need to compress?
        vectors = []

        for n in self.ansatz.split_cluster_ranks(spin=self.spin_type)[0]:
            for spin in util.generate_spin_combinations(n, unique=True):
                for key in keys:
                    vectors.append(getattr(getattr(amplitudes["t%d" % n], spin), key).ravel())
        ...
        for n in self.ansatz.split_cluster_ranks(spin=self.spin_type)[0]:
            ...
            for spin in util.generate_spin_combinations(n, unique=True):
                ...
                for key in keys:
                    occ = lambda space, k: space.naocc if k == "O" else space.niocc
                    vir = lambda space, k: space.navir if k == "O" else space.nivir
                    shape = tuple(occ(self.space["ab".index(s)], k) for s, k in zip(spin[:n], key[:n]))
                    shape += tuple(vir(self.space["ab".index(s)], k) for s, k in zip(spin[n:], key[n:]))
                    size = np.prod(shape)
                    tn = vector[i0 : i0 + size].reshape(shape)
                    setattr(getattr(amplitudes["t%d" % n], spin), key, tn)
                    i0 += size
