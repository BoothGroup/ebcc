...
# vo hole block (Operator with rank -3, therefore T^5 max)
L_expr = Expression([Term(1, [], [Tensor([a], "")], [FOperator(a, False)], [])])
R_expr = Expression([Term(1, [], [Tensor([i], "")], [FOperator(i, True)], [])])
...
print("P_{vo part} = ")
print(add_H_suffix_blocks(final._print_einsum('mom1_p_vo')))
print('***',flush=True)

# vv hole block (Operator with rank -2, therefore T^4 max)
L_expr = Expression([Term(1, [], [Tensor([a], "")], [FOperator(a, False)], [])])
R_expr = Expression([Term(1, [], [Tensor([b], "")], [FOperator(b, True)], [])])
expect = L_expr * (H - E0) * R_expr 
PT = commute(expect, T)
PTT = commute(PT, T)
PTTT = commute(PTT, T)
...
final = AExpression(Ex=out, simplify=simplify)
print("P_{vv part} = ")
print(add_H_suffix_blocks(final._print_einsum('mom1_p_vv')))
print('***',flush=True)

# oo hole block (Operator with rank -2, therefore T^4 max)
L_expr = Expression([Term(1, [], [Tensor([i], "")], [FOperator(i, False)], [])])
R_expr = Expression([Term(1, [], [Tensor([j], "")], [FOperator(j, True)], [])])
expect = L_expr * (H - E0) * R_expr 
PT = commute(expect, T)
PTT = commute(PT, T)
PTTT = commute(PTT, T)
...
final = AExpression(Ex=out, simplify=simplify)
print("P_{oo part} = ")
print(add_H_suffix_blocks(final._print_einsum('mom1_p_oo')))
print('***',flush=True)

# ov hole block (Operator with rank -1, therefore T^3 max)
L_expr = Expression([Term(1, [], [Tensor([i], "")], [FOperator(i, False)], [])])
R_expr = Expression([Term(1, [], [Tensor([a], "")], [FOperator(a, True)], [])])
expect = L_expr * (H - E0) * R_expr 
PT = commute(expect, T)
PTT = commute(PT, T)
PTTT = commute(PTT, T)
...
final = AExpression(Ex=out, simplify=simplify)
print("P_{ov part} = ")
print(add_H_suffix_blocks(final._print_einsum('mom1_p_ov')))
print('***',flush=True)
