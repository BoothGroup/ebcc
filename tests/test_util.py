...
class Util_Tests(unittest.TestCase):
    ...
    def test_constructors(self):
        ...
        ghf = uhf.to_ghf()
        log = ebcc.NullLogger()
        e_ccsd_r = ebcc.REBCC(rhf, fermion_ansatz="CCSD", e_tol=1e-10, log=log).kernel()
        e_ccsd_u = ebcc.UEBCC(uhf, fermion_ansatz="CCSD", e_tol=1e-10, log=log).kernel()
        e_ccsd_g = ebcc.GEBCC(ghf, fermion_ansatz="CCSD", e_tol=1e-10, log=log).kernel()
        ...
        e_cc2_r = ebcc.REBCC(rhf, fermion_ansatz="CC2", e_tol=1e-10, log=log).kernel()
        e_cc2_u = ebcc.UEBCC(uhf, fermion_ansatz="CC2", e_tol=1e-10, log=log).kernel()
        e_cc2_g = ebcc.GEBCC(ghf, fermion_ansatz="CC2", e_tol=1e-10, log=log).kernel()
